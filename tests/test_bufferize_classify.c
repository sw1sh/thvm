...
int main(void) {
  ...
  CHECK_EQ(bufferize_consumer_count(two), 2);
  ...
  CHECK_EQ(bufferize_consumer_count(pad), 2);
  CHECK_EQ(bufferize_is_realized(pad), 0);
  CHECK(bufferize_reasons(pad) & BUFFERIZE_REASON_MULTI);
  ...
  CHECK_EQ(bufferize_consumer_count(pure), 2);
