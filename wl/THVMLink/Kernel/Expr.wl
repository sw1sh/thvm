...
(* Expr.wl - WL <-> TTerm coercion.
   ...
   These were originally TLazyEncode / TLazyDecode in Lazy.wl, but
   they're not lazy themselves -- the laziness is in the Cons-stream
   structures the encoded values participate in.  Sit them in their
   own file so Lazy.wl can focus on stream combinators.  The
   underlying private workhorses (`tlazyEncode`, `tlazyDecode`,
   ...
   remain in Lazy.wl so the existing TLazyXxx generators can still
   reach them. *)
