...
SetUsage[TDef, "TDef[name$, body$] registers body$ as a named definition reachable via TRef[name$], and returns name$.
...
SetUsage[TRef, "TRef[name$] returns a TTerm wrapping a TAG_REF that lazily unfolds to the body registered under name$.
Reducing it via TWnf walks one ALO layer per fire (matching HVM4's REF-to-ALO unfolding)."];
SetUsage[TDefName, "TDefName[name$] returns the integer slot the string name$ is mapped to, interning it on first use; an integer name$ is returned unchanged."];
SetUsage[TDefSlots, "TDefSlots[] returns the current name-to-slot table as an Association."];
SetUsage[TDefGet, "TDefGet[name$] returns the registered body of a def as a book-heap TTerm, or Missing[\"UnregisteredDef\"] if the slot is empty.
name$ may be an integer slot or a string name."];
SetUsage[TDefExpr, "TDefExpr[name$] returns the structural expression of a def's body, walking the book heap (parallel to TTermExpr but for the immutable template).
Like TTermExpr, REF leaves stop expansion so self-referential defs render finitely."];
SetUsage[TDefTree, "TDefTree[name$] returns ExpressionTree[TDefExpr[name$]], a Wolfram Tree of the def body for visual inspection."];
...
$defRegisterFn := $defRegisterFn = load["thvm_wl_def_register", {Integer, Integer}, Integer]
$termNewRefFn := $termNewRefFn = load["thvm_wl_term_new_ref", {Integer}, Integer]
...
TDefGet[name_] := (
    ...
    With[{raw = $defGetFn[TDefName[name]]},
        If[raw === 0, Missing["UnregisteredDef", name], TTerm[raw]]
    ]
)

TDefExpr[name_] := With[{body = TDefGet[name]},
    If[MatchQ[body, _TTerm], tTreeWalkWith[$bookReadFn, body, <||>], body]
]
