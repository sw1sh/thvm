...
(* === lifecycle === *)
SetUsage[TInit, "TInit[] initializes the runtime, returning True."];
SetUsage[TFree, "TFree[] tears the runtime down."];
SetUsage[TReset, "TReset[] zeroes the heap, the WNF stack, and the interaction counter."];

(* === atomic term object === *)
SetUsage[TTerm, "TTerm[id$] wraps a packed 64-bit Term value.
...
SetUsage[TTermTag, "TTermTag[term$] returns the tag (an integer); accepts a TTerm or a raw integer."];
SetUsage[TTermExt, "TTermExt[term$] returns the EXT field."];
SetUsage[TTermVal, "TTermVal[term$] returns the VAL field (heap loc, etc.)."];
SetUsage[TTermSub, "TTermSub[term$] returns the SUB flag (0 or 1)."];
SetUsage[TTermUnpin, "TTermUnpin[term$] drops term$ from the extern-pinned-Terms GC root set.
...
SetUsage[TExternPinCount, "TExternPinCount[] returns the number of entries in the external-caller pin table, useful for observing that WL's GC has dropped TTerm wrappers between evaluations."];
SetUsage[TTagName, "TTagName[tag$] returns the name string for a tag id."];

(* === heap === *)
SetUsage[THeapPos, "THeapPos[] returns the next free heap location (the upper bound of the active heap region)."];
SetUsage[THeapBase, "THeapBase[] returns the lower bound of the active heap region.
...
SetUsage[THeapAlloc, "THeapAlloc[size$] reserves size$ consecutive cells and returns the base loc."];
SetUsage[THeapRead, "THeapRead[loc$] returns the Term at heap[loc$]."];
SetUsage[TBookRead, "TBookRead[loc$] returns the Term at book_heap[loc$], useful for inspecting CTR cells allocated via TBookCtr or built by the Metal AOT kernel."];
SetUsage[THeapSet, "THeapSet[loc$, term$] writes term$ to heap[loc$]."];
SetUsage[TGCCollect, "TGCCollect[] runs a Cheney semi-space collection of the dyn heap and returns the new live cell count."];
SetUsage[TGCCount, "TGCCount[] returns the number of GC cycles since thvm_init."];
...
SetUsage[THeap, "THeap[] returns an Association snapshot with keys \"nextLoc\", \"cells\", \"Graph\".  See docs/heap_graph.md."];
SetUsage[THeapGraph, "THeapGraph[] renders the heap state as an interaction-net string-diagram Graph.
...
SetUsage[THeapDiagram, "THeapDiagram[term$] builds a Wolfram`DiagrammaticComputation`DiagramNetwork from the heap, with one Diagram per compound agent and one ERA Diagram per ERA cell.
Wires share string identifiers keyed off heap loc; VAR cells collapse to their binder loc."];

(* === reduce / stats === *)
SetUsage[TWnf, "TWnf[term$] reduces term$ to weak normal form.
...
SetUsage[TNf, "TNf[term$] reduces term$ to full normal form: it nf-sweeps the live heap firing every redex, then cnfs the surviving root so the result is DP-free.
...
SetUsage[TCnf, "TCnf[term$] runs the cnf readback layer: reduces to weak head normal form then lifts the first SUP to the top, driving plain DP projections through their dup interactions.
Use it for a DP-free reading of a term without paying for nf's whole-heap sweep."];
SetUsage[TCollapse, "TCollapse[t$] enumerates the SUP-tree of t$, recursively walking SUP and ERA branches and collecting the non-SUP non-ERA leaves as a list of TTerms.
TCollapse[t$, cap$] caps the leaf count (default 65536)."];
SetUsage[TStep, "TStep[term$] = TWnf[term$, 1]: fires exactly one interaction.  Inspect TStack[] for the pending frames."];
SetUsage[TStack, "TStack[] returns the eliminator frames pending at the most recent bail point of TStep or TWnf[_, n$].
Each frame is a TTerm tagged APP, DP0, or DP1; the list is empty when no bail occurred."];
SetUsage[TRedexes, "TRedexes[] lists every redex in the live heap.
...
SetUsage[TInteract, "TInteract[redex$] fires exactly one interaction at redex$ and returns <|\"result\", \"fresh\"|>: \"result\" is the TTerm replacing the redex and \"fresh\" the redex-status flips this fire caused.
Returns Failure[\"NotARedex\", $$] when redex$ is no longer reducible."];
SetUsage[TReduce, "TReduce[term$] reduces term$ to WNF in-place and returns term$ (the original root, a useful seed for THeapGraph after reduction)."];
SetUsage[TItrs, "TItrs[] returns the cumulative interaction count."];
SetUsage[TTermExpr, "TTermExpr[term$] walks the heap from term$ and returns a nested expression whose heads are tag-name strings (\"LAM\", \"APP\", \"SUP\", \"DUP\", \"DP0\", \"DP1\", \"VAR\", \"ERA\").
Useful for snapshotting or diffing pre- and post-TWnf states by structural equality."];
SetUsage[TTermTree, "TTermTree[term$] = ExpressionTree[TTermExpr[term$]]: the same structure rendered as a Wolfram Tree object for visual inspection."];
SetUsage[TTermSubexprs, "TTermSubexprs[term$] returns a list of (path$, TTerm) rules covering every position reachable from term$ in pre-order DFS, where each path$ is a list of integer heap offsets (the root has empty path).
...
SetUsage[TSubexprAt, "TSubexprAt[term$, path$] navigates term$ along path$ (a list of integer offsets) and returns the subterm as a TTerm.
Returns Missing[\"OutOfBounds\", path$] when the route doesn't fit the term shape."];

(* === high-level constructors === *)
SetUsage[TFreshLabel, "TFreshLabel[] returns the next integer from a monotonic SUP/DUP label counter, then bumps it.  Reset by TReset[]."];
SetUsage[TEra, "TEra[] constructs an eraser term."];
SetUsage[TVarFor, "TVarFor[lamLoc$] constructs a VAR pointing at a binder loc."];
SetUsage[TLam, "TLam[x$, body$] constructs a lambda (HoldAll, so x$ is the binder symbol and body$ the lambda body referring to it, e.g. TLam[w, TUOpAdd[w, w]]).
...
SetUsage[TLamShape, "TLamShape[shape$, x$, body$] constructs a lambda whose bound variable carries an explicit shape annotation in the lam_shape side table.
...
SetUsage[TApp, "TApp[fun$, arg$] constructs an application."];
SetUsage[TSup, "TSup[a$, b$] constructs a SUP with a fresh label.
...
SetUsage[TDsu, "TDsu[label$, a$, b$] constructs a dynamic-label SUP (DSU): label$ is a TTerm reduced strict-left at wnf time, after which DSU collapses to SUP^n, ERA, or nested SUP based on the resolved label.
Useful for pattern compilers that need a fresh label per match instance."];
SetUsage[TDdu, "TDdu[label$, val$, body$] constructs a dynamic-label DUP (DDU), the DUP-side analogue of TDsu.
...
SetUsage[TTermEq, "TTermEq[a$, b$] returns True when a$ and b$ cnf-reduce to structurally equal terms (modulo VAR alpha-aliasing), False otherwise.
...
SetUsage[TTermSame, "TTermSame[a$, b$] returns True when a$ and b$ are structurally equal without further reduction, comparing tag/ext/val and recursing on children (same-loc compounds are trivially equal).
Cheap when callers already have CNF terms; for general use prefer TTermEq."];
SetUsage[TDup, "TDup[body$] constructs a DUP with a fresh label and returns the pair {dp0$, dp1$} (DP0/DP1 tags sharing one dup cell).
...
SetUsage[$TagAPP, "$TagAPP is a tag id; mirrors the corresponding TAG_* in src/thvm.h."];
SetUsage[$TagLAM, "$TagLAM is a tag id; mirrors the corresponding TAG_* in src/thvm.h."];
SetUsage[$TagVAR, "$TagVAR is a tag id; mirrors the corresponding TAG_* in src/thvm.h."];
SetUsage[$TagERA, "$TagERA is a tag id; mirrors the corresponding TAG_* in src/thvm.h."];
SetUsage[$TagDP0, "$TagDP0 is a tag id; mirrors the corresponding TAG_* in src/thvm.h."];
SetUsage[$TagDP1, "$TagDP1 is a tag id; mirrors the corresponding TAG_* in src/thvm.h."];
SetUsage[$TagSUP, "$TagSUP is a tag id; mirrors the corresponding TAG_* in src/thvm.h."];
SetUsage[$TagDUP, "$TagDUP is a tag id; mirrors the corresponding TAG_* in src/thvm.h."];
SetUsage[$TagTEN, "$TagTEN is a tag id; mirrors the corresponding TAG_* in src/thvm.h."];
SetUsage[$TagUOP, "$TagUOP is a tag id; mirrors the corresponding TAG_* in src/thvm.h."];
SetUsage[$TagNUM, "$TagNUM is a tag id; mirrors the corresponding TAG_* in src/thvm.h."];
SetUsage[$TagCTR, "$TagCTR is a tag id; mirrors the corresponding TAG_* in src/thvm.h."];
SetUsage[$TagREF, "$TagREF is a tag id; mirrors the corresponding TAG_* in src/thvm.h."];
SetUsage[$TagALO, "$TagALO is a tag id; mirrors the corresponding TAG_* in src/thvm.h."];
SetUsage[$TagOP2, "$TagOP2 is a tag id; mirrors the corresponding TAG_* in src/thvm.h."];
SetUsage[$TagMAT, "$TagMAT is a tag id; mirrors the corresponding TAG_* in src/thvm.h."];
SetUsage[$TagEQL, "$TagEQL is a tag id; mirrors the corresponding TAG_* in src/thvm.h."];
SetUsage[$TagAND, "$TagAND is a tag id; mirrors the corresponding TAG_* in src/thvm.h."];
SetUsage[$TagOR, "$TagOR is a tag id; mirrors the corresponding TAG_* in src/thvm.h."];
SetUsage[$TagANY, "$TagANY is a tag id; mirrors the corresponding TAG_* in src/thvm.h."];
SetUsage[$TagINC, "$TagINC is a tag id; mirrors the corresponding TAG_* in src/thvm.h."];
SetUsage[$TagWHEN, "$TagWHEN is a tag id; mirrors the corresponding TAG_* in src/thvm.h."];
SetUsage[$TagFVR, "$TagFVR is a tag id; mirrors the corresponding TAG_* in src/thvm.h."];
SetUsage[$TagBRI, "$TagBRI is a tag id; mirrors the corresponding TAG_* in src/thvm.h."];
SetUsage[$TagANN, "$TagANN is a tag id; mirrors the corresponding TAG_* in src/thvm.h."];
SetUsage[$TagPRI, "$TagPRI is a tag id; mirrors the corresponding TAG_* in src/thvm.h."];
SetUsage[$TagDSU, "$TagDSU is a tag id; mirrors the corresponding TAG_* in src/thvm.h."];
SetUsage[$TagDDU, "$TagDDU is a tag id; mirrors the corresponding TAG_* in src/thvm.h."];

(* === dtype + opcode constants (mirror src/thvm.h) === *)
SetUsage[$DTBool, "$DTBool is a dtype id; mirrors DT_* in src/thvm.h."];
SetUsage[$DTInt8, "$DTInt8 is a dtype id; mirrors DT_* in src/thvm.h."];
SetUsage[$DTUInt8, "$DTUInt8 is a dtype id; mirrors DT_* in src/thvm.h."];
SetUsage[$DTInt16, "$DTInt16 is a dtype id; mirrors DT_* in src/thvm.h."];
SetUsage[$DTUInt16, "$DTUInt16 is a dtype id; mirrors DT_* in src/thvm.h."];
SetUsage[$DTInt32, "$DTInt32 is a dtype id; mirrors DT_* in src/thvm.h."];
SetUsage[$DTUInt32, "$DTUInt32 is a dtype id; mirrors DT_* in src/thvm.h."];
SetUsage[$DTInt64, "$DTInt64 is a dtype id; mirrors DT_* in src/thvm.h."];
SetUsage[$DTUInt64, "$DTUInt64 is a dtype id; mirrors DT_* in src/thvm.h."];
SetUsage[$DTFp8E4M3, "$DTFp8E4M3 is a dtype id; mirrors DT_* in src/thvm.h."];
SetUsage[$DTFp8E5M2, "$DTFp8E5M2 is a dtype id; mirrors DT_* in src/thvm.h."];
SetUsage[$DTFp16, "$DTFp16 is a dtype id; mirrors DT_* in src/thvm.h."];
SetUsage[$DTBf16, "$DTBf16 is a dtype id; mirrors DT_* in src/thvm.h."];
SetUsage[$DTFp32, "$DTFp32 is a dtype id; mirrors DT_* in src/thvm.h."];
SetUsage[$DTFp64, "$DTFp64 is a dtype id; mirrors DT_* in src/thvm.h."];
SetUsage[$DTInt4, "$DTInt4 is a dtype id; mirrors DT_* in src/thvm.h."];
SetUsage[$DTUInt4, "$DTUInt4 is a dtype id; mirrors DT_* in src/thvm.h."];

SetUsage[$UopMaterialize, "$UopMaterialize is a UOp opcode id; mirrors UOP_* in src/thvm.h."];
SetUsage[$UopKernel, "$UopKernel is a UOp opcode id; mirrors UOP_* in src/thvm.h."];
SetUsage[$UopConst, "$UopConst is a UOp opcode id; mirrors UOP_* in src/thvm.h."];
SetUsage[$UopReshape, "$UopReshape is a UOp opcode id; mirrors UOP_* in src/thvm.h."];
SetUsage[$UopPermute, "$UopPermute is a UOp opcode id; mirrors UOP_* in src/thvm.h."];
SetUsage[$UopExpand, "$UopExpand is a UOp opcode id; mirrors UOP_* in src/thvm.h."];
SetUsage[$UopPad, "$UopPad is a UOp opcode id; mirrors UOP_* in src/thvm.h."];
SetUsage[$UopShrink, "$UopShrink is a UOp opcode id; mirrors UOP_* in src/thvm.h."];
SetUsage[$UopFlip, "$UopFlip is a UOp opcode id; mirrors UOP_* in src/thvm.h."];
SetUsage[$UopAdd, "$UopAdd is a UOp opcode id; mirrors UOP_* in src/thvm.h."];
SetUsage[$UopMul, "$UopMul is a UOp opcode id; mirrors UOP_* in src/thvm.h."];
SetUsage[$UopNeg, "$UopNeg is a UOp opcode id; mirrors UOP_* in src/thvm.h."];
SetUsage[$UopRecip, "$UopRecip is a UOp opcode id; mirrors UOP_* in src/thvm.h."];
SetUsage[$UopExp2, "$UopExp2 is a UOp opcode id; mirrors UOP_* in src/thvm.h."];
SetUsage[$UopLog2, "$UopLog2 is a UOp opcode id; mirrors UOP_* in src/thvm.h."];
SetUsage[$UopSqrt, "$UopSqrt is a UOp opcode id; mirrors UOP_* in src/thvm.h."];
SetUsage[$UopCmplt, "$UopCmplt is a UOp opcode id; mirrors UOP_* in src/thvm.h."];
SetUsage[$UopReduce, "$UopReduce is a UOp opcode id; mirrors UOP_* in src/thvm.h."];
SetUsage[$UopGrad, "$UopGrad is a UOp opcode id; mirrors UOP_* in src/thvm.h."];
SetUsage[$UopCmpeq, "$UopCmpeq is a UOp opcode id; mirrors UOP_* in src/thvm.h."];
SetUsage[$UopLoad, "$UopLoad is a UOp opcode id; mirrors UOP_* in src/thvm.h."];
SetUsage[$UopAssign, "$UopAssign is a UOp opcode id; mirrors UOP_* in src/thvm.h."];
SetUsage[$UopCast, "$UopCast is a UOp opcode id; mirrors UOP_* in src/thvm.h."];
SetUsage[$UopBitcast, "$UopBitcast is a UOp opcode id; mirrors UOP_* in src/thvm.h."];
SetUsage[$UopRange, "$UopRange is a UOp opcode id; mirrors UOP_* in src/thvm.h."];
SetUsage[$UopIndexE, "$UopIndexE is a UOp opcode id; mirrors UOP_* in src/thvm.h."];
SetUsage[$UopIAdd, "$UopIAdd is a UOp opcode id; mirrors UOP_* in src/thvm.h."];
SetUsage[$UopISub, "$UopISub is a UOp opcode id; mirrors UOP_* in src/thvm.h."];
SetUsage[$UopIMul, "$UopIMul is a UOp opcode id; mirrors UOP_* in src/thvm.h."];
SetUsage[$UopIDiv, "$UopIDiv is a UOp opcode id; mirrors UOP_* in src/thvm.h."];
SetUsage[$UopIMod, "$UopIMod is a UOp opcode id; mirrors UOP_* in src/thvm.h."];
SetUsage[$UopILt, "$UopILt is a UOp opcode id; mirrors UOP_* in src/thvm.h."];
SetUsage[$UopIAnd, "$UopIAnd is a UOp opcode id; mirrors UOP_* in src/thvm.h."];
SetUsage[$UopIWhere, "$UopIWhere is a UOp opcode id; mirrors UOP_* in src/thvm.h."];
SetUsage[$UopInvalid, "$UopInvalid is a UOp opcode id; mirrors UOP_* in src/thvm.h."];
SetUsage[$UopBuffer, "$UopBuffer is a UOp opcode id; mirrors UOP_* in src/thvm.h."];
SetUsage[$UopStore, "$UopStore is a UOp opcode id; mirrors UOP_* in src/thvm.h."];
SetUsage[$UopAfter, "$UopAfter is a UOp opcode id; mirrors UOP_* in src/thvm.h."];
SetUsage[$UopOpt, "$UopOpt is a UOp opcode id; mirrors UOP_* in src/thvm.h."];

SetUsage[$ReduceSum, "$ReduceSum is a reduce-kind id; mirrors REDUCE_* in src/thvm.h."];
SetUsage[$ReduceMax, "$ReduceMax is a reduce-kind id; mirrors REDUCE_* in src/thvm.h."];

SetUsage[$UopScopeGlobal, "$UopScopeGlobal is a UOP_BUFFER scope tag; mirrors UOP_SCOPE_GLOBAL/LOCAL/REG in src/thvm.h."];
SetUsage[$UopScopeLocal, "$UopScopeLocal is a UOP_BUFFER scope tag; mirrors UOP_SCOPE_GLOBAL/LOCAL/REG in src/thvm.h."];
SetUsage[$UopScopeReg, "$UopScopeReg is a UOP_BUFFER scope tag; mirrors UOP_SCOPE_GLOBAL/LOCAL/REG in src/thvm.h."];

(* === tensors === *)
SetUsage[TTensor, "TTensor[shape$, dtype$] allocates a tensor and returns a TTerm wrapping a TEN handle (dtype$ defaults to \"f32\").
TTensor[shape$, data$] also writes initial values."];
SetUsage[TTensorCreate, "TTensorCreate[data$] builds a TTerm tensor whose shape and dtype are inferred from data$.
...
SetUsage[TTensorCreateHost, "TTensorCreateHost[data$] builds a TTerm tensor that is always CPU-resident (zero-copy NumericArray wrap) regardless of the active backend.
...
SetUsage[TTensorShape, "TTensorShape[t$] returns the tensor's shape as a list of integers."];
SetUsage[TTermShape, "TTermShape[t$] runs the runtime's term_shape_in inference, returning a list of dim extents for a TEN, a shape-inferable UOP, or a VAR with a registered shape annotation (TLamShape).
Returns {} when the shape cannot be determined."];
SetUsage[TTensorDType, "TTensorDType[t$] returns the dtype as a string (\"f32\" or \"i32\")."];
SetUsage[TTensorData, "TTensorData[t$] reads the tensor's buffer as a NumericArray matching the dtype (Real32 for f32, Integer32 for i32).  Wrap in Normal to get a plain list."];
SetUsage[TTensorRefcount, "TTensorRefcount[t$] returns the descriptor refcount."];
SetUsage[TGradOf, "TGradOf[t$] returns the lazy grad term (the chain-rule accumulator populated by a TGrad[loss] or TUOpGrad[y, gy] realize).
Returns Missing[\"NoGrad\"] when no grad has been accumulated; wrap with TRealize to materialize."];
SetUsage[TClearGrad, "TClearGrad[t$] zeros the grad accumulator (the zero_grad analogue) and returns t$."];
SetUsage[TRealize, "TRealize[expr$] = TWnf[TMaterialize[expr$]]: fires the whole pipeline, heap-walk materializing UOPs to kernels then beta-reducing and dispatching them."];
SetUsage[TMaterialize, "TMaterialize[expr$] runs the schedule, kernelize, and linearize rewrite directly (no wnf) and returns the scheduled DAG term, firing no kernels.  Use it to visualize the graph after scheduling but before dispatch."];
...
SetUsage[TUOpConst, "TUOpConst[value$, dtype$] builds a UOP_CONST node carrying a scalar."];
SetUsage[TUOpReshape, "TUOpReshape[src$, shape$] builds a UOP_RESHAPE node."];
SetUsage[TUOpPermute, "TUOpPermute[src$, axes$] builds a UOP_PERMUTE node."];
SetUsage[TUOpExpand, "TUOpExpand[src$, shape$] builds a UOP_EXPAND node."];
SetUsage[TUOpPad, "TUOpPad[src$, ranges$] builds a UOP_PAD node, where ranges$ is {{b$0, e$0}, {b$1, e$1}, $$}."];
SetUsage[TUOpShrink, "TUOpShrink[src$, ranges$] builds a UOP_SHRINK node."];
SetUsage[TUOpFlip, "TUOpFlip[src$, axes$] builds a UOP_FLIP node, where axes$ is a list of axis indices to flip."];
SetUsage[TUOpAdd, "TUOpAdd[a$, b$] builds a UOP_ADD node."];
SetUsage[TUOpMul, "TUOpMul[a$, b$] builds a UOP_MUL node."];
SetUsage[TUOpNeg, "TUOpNeg[a$] builds a UOP_NEG node."];
SetUsage[TUOpRecip, "TUOpRecip[a$] builds a UOP_RECIP node."];
SetUsage[TUOpExp2, "TUOpExp2[a$] builds a UOP_EXP2 node."];
SetUsage[TUOpLog2, "TUOpLog2[a$] builds a UOP_LOG2 node."];
SetUsage[TUOpSqrt, "TUOpSqrt[a$] builds a UOP_SQRT node."];
SetUsage[TUOpCmplt, "TUOpCmplt[a$, b$] builds a UOP_CMPLT node."];
SetUsage[TUOpCmpeq, "TUOpCmpeq[a$, b$] builds a UOP_CMPEQ node (the elementwise a$ == b$ mask)."];
SetUsage[TUOpReduce, "TUOpReduce[src$, axis$, kind$] builds a UOP_REDUCE node, where kind$ is \"SUM\" or \"MAX\"."];
SetUsage[TKVarAlloc, "TKVarAlloc[lo$, hi$] allocates a symbolic-shape dimension variable in [lo$, hi$] and returns its integer id.
Bind it with TKVarSet before TRealize and mark a tensor axis symbolic with TSymbolicAxis."];
SetUsage[TKVarHi, "TKVarHi[vid$] returns the upper bound vid$ was allocated with (the static extent its buffers carry), used to size host constants whose leading axis is then marked symbolic on vid$."];
SetUsage[TKVarSet, "TKVarSet[vid$, value$] binds the symbolic dimension vid$ to value$ (the loop bound) for the next TRealize; buffers stay sized at the upper bound."];
SetUsage[TSymbolicAxis, "TSymbolicAxis[t$, axis$, vid$] reinterprets tensor t$'s axis as the symbolic dimension vid$ (the axis's current extent must equal the kvar upper bound), so the lifted forward runs that axis at the bound TKVarSet value."];
SetUsage[TAppendAt, "TAppendAt[cache$, src$, posVid$] writes src$ (a {1, $$} row) into cache$ ({nCtx, $$}) in place at the runtime row offset bound to kvar posVid$, leaving other rows untouched (the KV-cache append).
Returns cache$, whose buffer is mutated so callers holding it see the write."];
SetUsage[TDecodeAttend, "TDecodeAttend[q1$, kCache$, vCache$, kNew$, vNew$, nHeads$, posVid$, lenVid$, scale$] is GPT-2's per-step decode attention over a KV cache (T=1).
...
SetUsage[TDecodeInit, "TDecodeInit[net$] allocates an incremental-decode session for a token-LM NetChain (GPT-2): one zeroed {nCtx, dim} KV cache per attention block, the two position kvars (posVid, lenVid), and write position 0.
...
SetUsage[TDecodeNext, "TDecodeNext[net$, state$, tokenId$] drives one decode step: it feeds integer tokenId$ at the session's current position, appends its K/V into every block's cache, attends, and returns the updated state$ with \"pos\" advanced, \"token\" the greedy next-token id, and \"logits\" the {vocab} logits.
Chain it over a prompt then on its own \"token\" output to generate; state$ comes from TDecodeInit."];
SetUsage[TUOpCast, "TUOpCast[src$, dtype$] builds a UOP_CAST node, where dtype$ is one of \"f32\", \"i32\", \"i8\", etc.  Its backward gradient (under TGrad) is a cast back to src$'s dtype."];
SetUsage[TUOpGrad, "TUOpGrad[y$, gy$] builds the backward projection of a dup-flavored grad cell holding [y$, gy$], where gy$ is the cotangent (must match y$'s shape).
...
SetUsage[TUOpLoad, "TUOpLoad[src$] builds a UOP_LOAD node wrapping src$, a structural marker mirroring tinygrad's LOAD whose runtime semantics are identity (memcpy in the cpu kernel)."];
SetUsage[TUOpCopy, "TUOpCopy[src$] builds a UOP_COPY node: a lazy device transfer that uploads src$ to the realize backend at materialize time.
...
SetUsage[TDevice, "TDevice[t$] returns the device a TTerm's data lives on (\"cpu\", \"metal\", $$), or None when it inherits the default device.
...
SetUsage[TToDevice, "TToDevice[t$, dev$] (dev$ is \"metal\" or \"cpu\") moves t$ to a device (tinygrad's Tensor.to).
...
SetUsage[TUOpRange, "TUOpRange[axisId$, axisType$, extent$] builds a UOP_RANGE leaf, where axisType$ is one of $KaxLoop, $KaxReduce, $KaxUpcast, $KaxUnroll, $KaxLocal, $KaxGlobal, $KaxGroupReduce."];
SetUsage[TUOpBuffer, "TUOpBuffer[scope$, dtype$, dims$, instance$] builds a UOP_BUFFER node, where scope$ is $UopScopeGlobal/Local/Reg, dtype$ is the dtype id, dims$ is a list of integers, and instance$ disambiguates input slots (0 = output, 1.. = inputs)."];
SetUsage[TUOpIndexE, "TUOpIndexE[buf$, addr$] builds a UOP_INDEX_E node pairing a buffer with a symbolic address tree."];
SetUsage[TUOpIAdd, "TUOpIAdd[a$, b$] builds the integer-add UOP node, used to construct symbolic address trees over UOP_RANGE leaves and UOP_CONST stride coefficients."];
SetUsage[TUOpISub, "TUOpISub[a$, b$] builds the integer-subtract UOP node, used to construct symbolic address trees over UOP_RANGE leaves and UOP_CONST stride coefficients."];
SetUsage[TUOpIMul, "TUOpIMul[a$, b$] builds the integer-multiply UOP node, used to construct symbolic address trees over UOP_RANGE leaves and UOP_CONST stride coefficients."];
SetUsage[TUOpIDiv, "TUOpIDiv[a$, b$] builds the integer-divide UOP node, used to construct symbolic address trees over UOP_RANGE leaves and UOP_CONST stride coefficients."];
SetUsage[TUOpIMod, "TUOpIMod[a$, b$] builds the integer-modulo UOP node, used to construct symbolic address trees over UOP_RANGE leaves and UOP_CONST stride coefficients."];
SetUsage[TUOpILt, "TUOpILt[a$, b$] builds the integer-less-than UOP node, used to construct symbolic address trees over UOP_RANGE leaves and UOP_CONST stride coefficients."];
SetUsage[TUOpIAnd, "TUOpIAnd[a$, b$] builds the integer-and UOP node, used to construct symbolic address trees over UOP_RANGE leaves and UOP_CONST stride coefficients."];
SetUsage[TUOpIWhere, "TUOpIWhere[cond$, then$, else$] builds a UOP_IWHERE ternary select."];
SetUsage[TUOpInvalid, "TUOpInvalid[] builds a UOP_INVALID sentinel (used for PAD-mask padding)."];
SetUsage[TUOpOpt, "TUOpOpt[target$, kind$, factor$] wraps target$ with a UOP_OPT annotation, where kind$ is one of $OptUnroll, $OptUpcast, $OptTC, $OptLocal, $OptGroupReduce."];
SetUsage[TUOpStore, "TUOpStore[buf$, addr$, value$] builds a UOP_STORE root, the typical kernel root STORE(out_buf, out_addr, value-expression)."];
SetUsage[TUOpAfter, "TUOpAfter[node$, afterNode$] builds a UOP_AFTER ordering edge: node$ happens after afterNode$."];
SetUsage[TUOpIConst, "TUOpIConst[v$] builds a UOP_CONST(DT_INT32) for use as a stride coefficient (the DAG-side classifiers pattern-match on UOP_CONST, not on bare NUM atoms)."];

SetUsage[TAssign, "TAssign[dst$, src$] builds a UOP_ASSIGN node, a wnf-fired in-place buffer write: once src$ reduces to a TEN, the backend memcpy copies src$'s buffer into dst$'s and the redex rewrites to dst$.
...
SetUsage[TGrad, "TGrad[y$] is loss.backward(): one backward walk seeded with ones-at-y$ that auto-grads every reachable non-CONST float leaf, accumulating each leaf's gradient into its grad slot (read back per tensor with TGradOf).
...
SetUsage[TUOpKind, "TUOpKind[u$] returns the opcode name for a UOp term."];
SetUsage[TUOpSrcs, "TUOpSrcs[u$] returns the source-cell terms for a UOp term, in heap order."];
...
$libDir = FileNameJoin[{DirectoryName[$InputFileName], "..", "LibraryResources", $SystemID}];
...
$lib = If[
    $SystemID === "Linux-x86-64" && FileExistsQ[$libCuda] && StringQ[Quiet @ LibraryLoad[$libCuda]],
    $libCuda,
    $libCpu
];
...
$externPinAssociateFn := $externPinAssociateFn = load["thvm_wl_extern_pin_associate", {Integer, Integer}, Integer];
$externPinCountFn := $externPinCountFn = load["thvm_wl_extern_pin_count", {}, Integer];
$externPinHandleGetFn := $externPinHandleGetFn = load["thvm_wl_extern_pin_handle_get", {Integer}, Integer];
...
$labelCounter /: Set[$labelCounter, n_Integer] := ($labelCounters[$contextCurrentFn[]] = n; n)
$labelCounter := Lookup[$labelCounters, $contextCurrentFn[], 1]

(* === public API === *)
$initializedContexts = <||>
$initialized /: Set[$initialized, v_] := ($initializedContexts[$contextCurrentFn[]] = TrueQ[v]; v)
...
TInit[] := (
    $labelCounters[$contextCurrentFn[]] = 1;
    $initializedContexts[$contextCurrentFn[]] = True;
    $initFn[] === 1
)
TFree[] := (
    $initializedContexts[$contextCurrentFn[]] = False;
    $freeFn[]
)
TReset[] := (
    $labelCounters[$contextCurrentFn[]] = 1;
...
ttermRaw[TTerm[_Integer, id_Integer, h_]] := With[{fresh = $externPinHandleGetFn[ManagedLibraryExpressionID[h]]},
    If[ fresh =!= 0, fresh, id]
]
...
makePinHandle[raw_Integer] := With[{h = CreateManagedLibraryExpression["ExternPin", ExternPin]},
...
TTerm /: Equal[TTerm[c1_Integer, r1_Integer, _], TTerm[c2_Integer, r2_Integer, _]] := c1 === c2 && r1 === r2
...
packTerm[sub_Integer, tag_Integer, ext_Integer, val_Integer] := TTerm[$termNewFn[sub, tag, ext, val]]
...
TInteract[redex_TTerm, roots___] := withTermCtx[redex, Module[{redexRaw, rootRaws, result, freshN, owned},
    ensureInit[];
    ...
    If[ result === 0,
        If[ owned, $stepEndFn[] ];
        Return @ Failure["NotARedex", <| "Message" -> "TInteract: cell is not a redex right now", "redex" -> redex |>]
    ];
    freshN = $stepFreshFn[];
    Module[{res = <| "result" -> TTerm[result], "fresh" -> TTerm /@ Table[$redexGetFn[i], {i, 0, freshN - 1}] |>},
        If[ owned, $stepEndFn[] ];
        res
    ]
]]
...
(* (offset, child-raw) pairs for a heap term's structural children.
   ...
   metadata). *)
subexprChildren[raw_Integer] := Block[{tag = $termTagFn[raw], val = $termValFn[raw], ext = $termExtFn[raw], n},
    Switch[ tag,
...
walkSubexprsRaw[raw_Integer, path_List] := Block[{children = subexprChildren[raw]},
    Prepend[
        Catenate[
...
TSubexprAt[t_TTerm, path_List] := Block[{raw = ttermRaw[t], k, ch},
    Catch[
        Do[
...
tTreeWalkWith[reader_, t_, seen_] := Block[{raw = ttermRaw[t], tag, val, ext, seen2, n},
    tag = $termTagFn[raw];
    val = $termValFn[raw];
    ext = $termExtFn[raw];
    Switch[tag,
        ...
        $TagUOP,
            If[ KeyExistsQ[seen, val], "Cycle"[val],
                ...
                If[ ext === $UopKernel,
                    ...
                    Module[{kid = $termValFn[reader[val + 1]], outTid = $termValFn[reader[val]], inTids},
...
heapTerm[tag_Integer, ext_Integer, fields__] := packTerm[0, tag, ext, heapWith[fields]]
...
TTermEq[a_TTerm, b_TTerm] := (ensureInit[]; $termEqFn[ttermRaw[a], ttermRaw[b]] === 1)
TTermEq[a_Integer, b_Integer] := (ensureInit[]; $termEqFn[a, b] === 1)

TTermSame[a_TTerm, b_TTerm] := (ensureInit[]; $termEqStructFn[ttermRaw[a], ttermRaw[b]] === 1)
TTermSame[a_Integer, b_Integer] := (ensureInit[]; $termEqStructFn[a, b] === 1)
...
TDup[{dp0_TTerm, dp1_TTerm}] /; (
    TTermTag[dp0] === $TagDP0 && TTermTag[dp1] === $TagDP1 && TTermExt[dp0] === TTermExt[dp1] && TTermVal[dp0] === TTermVal[dp1]) :=
