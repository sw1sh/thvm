...
| `Kernel.wl`         | `TKernel` typed wrapper + property surface (`Information[k]`, `k["Name"]`, `k[]`). Owns the kernel side-table accessors `TKernelTable`, `TKernelInputs`, plus the kid-keyed accessors: `TKernelCount`, `TKernelInfo`, `TKernelProgramCacheSize`, `TKernelSource[kid, backend]`, `TKernelFlops`, `TKernelDispatchKind`/`Count`/`TotalUs`, `TKernelJitDylibPath`, `TKernelProfile`, `TProfileAll`. Also `TOpt[op, axis, arg]` + `TKernelOpts[<\|...\|>]` (wrapped) + `TKernelApplyOpt[kid, opt]` + `TKernelProposed[kid]` + `TKernelAutotune[kid]` -- thin LibraryLink wrappers over the C-side `KernelAxes` struct, proposer (`src/codegen/propose.c`), and autotune loop (`src/codegen/autotune.c`); WL is the surface, C is the source of truth. |
