...
| `Kernel.wl`         | `TKernel` typed wrapper + property surface (`Information[k]`, `k["Name"]`, `k[]`). Top-level kid-keyed accessors: `TKernelCount`, `TKernelInfo`, `TKernelProgramCacheSize`, `TKernelSource[kid, backend]`, `TKernelFlops`, `TKernelDispatchKind`/`Count`/`TotalUs`, `TKernelJitDylibPath`, `TKernelProfile`, `TProfileAll`. |
