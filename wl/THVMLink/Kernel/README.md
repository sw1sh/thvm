...
| `Kernel.wl`         | `TKernel` typed wrapper + property surface (`Information[k]`, `k["Name"]`, `k[]`). Owns the kernel side-table accessors `TKernelTable`, `TKernelInputs`, plus the kid-keyed accessors: `TKernelCount`, `TKernelInfo`, `TKernelProgramCacheSize`, `TKernelSource[kid, backend]`, `TKernelFlops`, `TKernelDispatchKind`/`Count`/`TotalUs`, `TKernelJitDylibPath`, `TKernelProfile`, `TProfileAll`. Also `Opt[op, axis, arg]` + `TKernelOpts` + `TKernelApplyOpt` axis-types view. |
