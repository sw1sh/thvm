...
SetUsage[TMultiHeadAttention, "TMultiHeadAttention[Q$, K$, V$, nHeads$, mask$] computes multi-head scaled dot-product attention. Q$ is {seq_q, dim}; K$, V$ are {seq_k, dim} (read independently, so seq_k may differ for cross-attention or a single-query step over a KV cache); dim = nHeads$ * d_head; mask$ is a {seq_q, seq_k} additive bias (use TCausalMask) or None. Each projection is split to {nHeads$, seq, d_head}, scaled-dot per head, concatenated back to {seq_q, dim}.
...
SetUsage[TDecodeAttend, "TDecodeAttend[q1$, kCache$, vCache$, kNew$, vNew$, nHeads$, posVid$, lenVid$, scale$] is GPT-2 per-step decode attention over a KV cache. It appends kNew$/vNew$ into kCache$/vCache$ at row posVid$, marks the {len, dim} prefix symbolic on lenVid$, and attends the single query q1$ ({1, dim}) over the cached prefix multi-head, returning {1, dim}. See the public symbol page."];
SetUsage[TCausalMask, "TCausalMask[seq$] returns a {seq$, seq$} TTerm whose entries are 0 at (i, j) with j <= i and -1e9 at j > i. Add it to attention scores before softmax to zero out future-token attention."];
SetUsage[TCausalMaskSym, "TCausalMaskSym[vid$, hi$] returns the symbolic-sequence causal mask: a {S, S} TTerm (S the kvar dimension vid$, upper bound hi$) with 0 at (i, j) for j <= i and a large negative bias at j > i. Built eagerly from a symbolic ramp so a symbolic-seq TMultiHeadAttention can add it to scores before softmax. Use in place of TCausalMask[seq] when the sequence axis is a kvar, passing the result as TMultiHeadAttention's mask argument."];
SetUsage[TPaddingCausalMask, "TPaddingCausalMask[attentionMask$] returns the {S, S} additive causal+padding bias for a host attention mask attentionMask$ (a length-S list, 1 for a real token / 0 for padding): 0 where token i may attend to j (j <= i causal AND attentionMask$[j] real), a large negative bias elsewhere.
TPaddingCausalMask[attentionMask$, neg$] uses neg$ as the masked-out bias (default -1e9)."];
SetUsage[TRoPEInterleaved, "TRoPEInterleaved[x$, cos$, sin$] applies the interleaved rotary embedding (the FLUX.2 DiT convention, use_real_unbind_dim=-1) to x$ {S, H, D}: adjacent pairs (x$[2i], x$[2i+1]) rotate to (-x$[2i+1], x$[2i]); cos$, sin$ are {S, 1, D} broadcast over heads. The Qwen3 text encoder uses TRoPEHalfSplit instead -- the two conventions are NOT interchangeable."];
SetUsage[TRoPEHalfSplit, "TRoPEHalfSplit[x$, cos$, sin$] applies the half-split rotary embedding (the Qwen3 / GPT-NeoX rotate_half convention) to x$ {S, H, D}: rotate_half(x$) = concat[-x$[D/2:], x$[:D/2]], y = x$ cos$ + rotate_half(x$) sin$; cos$, sin$ are {S, 1, D} broadcast over heads. The FLUX DiT uses TRoPEInterleaved instead."];
SetUsage[TRoPEHalfSplitTable, "TRoPEHalfSplitTable[seq$, headDim$, theta$] builds the half-split (NEOX) rotary cos/sin tables for the Qwen3 convention: inv_freq[i] = theta$^(-2i/headDim$), freqs = outer(pos, inv_freq), emb = concat[freqs, freqs]. Returns {cos, sin} each {seq$, 1, headDim$} so they broadcast over the head axis, ready for TRoPEHalfSplit."];
SetUsage[TRepeatKV, "TRepeatKV[x$, rep$] is the grouped-query-attention head expansion: a {S, Hkv, D} tensor whose Hkv kv heads each serve rep$ query heads, broadcast to {S, Hkv*rep$, D} in HF repeat_interleave order (kv head j serves query heads rep$*j .. rep$*j+rep$-1). Insert a unit axis, expand it to rep$, fold back so the repeats land contiguously per kv head."];
...
mhaBmm[a_, bm_, bb_, m_, kk_, nn_] := TUOpReduce[TUOpMul[TUOpExpand[TUOpReshape[a, {bb, m, kk, 1}], {bb, m, kk, nn}], TUOpExpand[TUOpReshape[bm, {bb, 1, kk, nn}], {bb, m, kk, nn}]], 2, "SUM"]
...
(* M-major batched matmul: A:{B,M,K} . Bm:{B,K,N} -> {M,B,N} (batch axis in the
   ...
   BLAS classifier. *)
mhaBmmM[a_, bm_, bb_, m_, kk_, nn_] := TUOpReduce[TUOpMul[TUOpExpand[TUOpReshape[Transpose[a, {2, 1, 3}], {m, bb, kk, 1}], {m, bb, kk, nn}], TUOpExpand[TUOpReshape[bm, {1, bb, kk, nn}], {m, bb, kk, nn}]], 2, "SUM"]
...
(* THeadAttention[q, k, v, scale, addMask]: scaled-dot attention over
   ...
   None for unmasked. *)
THeadAttention[q_TTerm, k_TTerm, v_TTerm, scale_ ? NumericQ] := THeadAttention[q, k, v, scale, None]
THeadAttention[q_TTerm, k_TTerm, v_TTerm, scale_ ? NumericQ, addMask_] := Module[{sq, h, dh, sk, qh, kh, vh, scores, scoresM, attn, out},
    {sq, h, dh} = tUopShape[q];
    sk = tUopShape[k][[1]];
    qh = Transpose[q, {2, 1, 3}];  kh = Transpose[k, {2, 1, 3}];  vh = Transpose[v, {2, 1, 3}];
    scores = mhaBmm[qh, Transpose[kh, {1, 3, 2}], h, sq, dh, sk] * scale;
    scoresM = If[ addMask === None, scores, scores + TUOpExpand[ArrayReshape[addMask, {1, sq, sk}], {h, sq, sk}]];
...
TRoPEInterleaved[x_TTerm, cos_TTerm, sin_TTerm] := Module[{s, h, d, xr, xe, xo, xrot},
    ...
    xrot = TUOpReshape[Join[TUOpReshape[-xo, {s, h, d/2, 1}], TUOpReshape[xe, {s, h, d/2, 1}], 4], {s, h, d}];
...
TRoPEHalfSplitTable[seq_Integer, headDim_Integer, theta_ ? NumericQ] := Module[{half, invFreq, emb},
    half = headDim/2;
...
TCausalMask[seq_Integer] := TTensorCreate @ NumericArray[Table[If[j <= i, 0.0, -1.0*^9], {i, seq}, {j, seq}], "Real32"]
...
TPaddingCausalMask[attentionMask_List, neg_ : -1.*^9] := With[{s = Length[attentionMask]},
    TTensorCreate[N @ Table[If[j <= i && attentionMask[[j]] == 1, 0., neg], {i, s}, {j, s}]]
]
...
TCausalMaskSym[vid_Integer, hi_Integer] := Module[{sym, er, ramp, ri, rj},
    ...
    er @ TUOpMul[TUOpCmplt[TUOpAdd[ri, TUOpNeg[rj]], TUOpConst[0.]], TUOpConst[-30.]]
]
...
(* TAppendAt[cache, src, posVid] -- the in-place KV-cache APPEND: write `src`
   ...
   returns `cache`. *)
TAppendAt[cache_TTerm, src_TTerm, posVid_Integer] := Module[{p = TKVarPack[posVid], tailRanges},
...
TDecodeAttend[q1_TTerm, kCache_TTerm, vCache_TTerm, kNew_TTerm, vNew_TTerm, nHeads_Integer, posVid_Integer, lenVid_Integer, scale_ ? NumericQ] := (
    TAppendAt[kCache, kNew, posVid];
    TAppendAt[vCache, vNew, posVid];
    TMultiHeadAttention[q1, TSymbolicAxis[kCache, 0, lenVid], TSymbolicAxis[vCache, 0, lenVid], nHeads, None, scale]
)

(* === multi-head attention (full projection split) ======================== *)

TMultiHeadAttention[q_TTerm, k_TTerm, v_TTerm, nHeads_Integer, mask_ : None] := With[{dHead = tUopShape[q][[2]] / nHeads},
...
TMultiHeadAttention[q_TTerm, k_TTerm, v_TTerm, nHeads_Integer, mask_, scale_ ? NumericQ] := With[{shapeQ = tUopShape[q], shapeK = tUopShape[k]},
    Module[{seqQ, seqK, dim, dHead},
        ...
        If[ symLeadingQ[q] || symLeadingQ[k],
            ...
            Module[{sliceHead, perHead},
                sliceHead[hView_, h_, s_] := ArrayReshape[TUOpShrink[hView, {{h, h + 1}, {0, s}, {0, dHead}}], {s, dHead}];
                perHead[h_] := With[{qS = sliceHead[mhaHeadView[q, seqQ, nHeads, dHead], h, seqQ], kS = sliceHead[mhaHeadView[k, seqK, nHeads, dHead], h, seqK], vS = sliceHead[mhaHeadView[v, seqK, nHeads, dHead], h, seqK]},
            ...
            Module[{qh, kh, vh, scores, scoresM, ctx},
                ...
                scoresM = If[ mask === None, scores, scores + TUOpExpand[TUOpReshape[mask, {1, seqQ, seqK}], {nHeads, seqQ, seqK}]];
...
headStitch[heads_List, nHeads_, dHead_] := Total @ Table[TUOpPad[heads[[h + 1]], {{0, 0}, {h * dHead, (nHeads - h - 1) * dHead}}], {h, 0, nHeads - 1}]
