...
SetUsage[TLayerNorm, "TLayerNorm[x$] and TLayerNorm[x$, eps$] normalise along the last axis: y = (x$ - mean) / sqrt(var + eps$). Default eps$ = 1e-5. mean and var are per-row reductions broadcast back via the softmax-style reduce-broadcast pattern."];
SetUsage[TLayerNormAffine, "TLayerNormAffine[x$, gamma$, beta$] applies TLayerNorm[x$] then multiplies by gamma$ and adds beta$ along the last axis. GPT-2's layer-norm carries learned gamma/beta; bare TLayerNorm normalises only."];
SetUsage[TLayerNormAffine, "TLayerNormAffine[x$, gamma$, beta$] applies TLayerNorm[x$] then multiplies by gamma$ and adds beta$ along the last axis. GPT-2's layer-norm carries learned gamma/beta; bare TLayerNorm normalises only."];
SetUsage[TRMSNorm, "TRMSNorm[x$, weight$, eps$] is RMS normalisation over the last axis: y = x$ * rsqrt(mean(x$^2) + eps$) * weight$, with weight$ a rank-1 {D} gain over the feature axis. The per-head q/k norm of FLUX / Qwen3 attention and the LLaMA-style pre-norm. No mean-subtraction (unlike LayerNorm)."];
SetUsage[TGroupNorm, "TGroupNorm[x$, weight$, bias$, groups$, eps$] is GroupNorm over a {C, H, W} tensor: split C into groups$ groups, normalise each group over its (C/groups$, H, W) elements, then apply a per-channel affine weight$/bias$ ({C}). The VAE / U-Net spatial normalisation."];
SetUsage[TBatchNorm, "TBatchNorm[x$, gamma$, beta$, mean$, var$] and TBatchNorm[x$, gamma$, beta$, mean$, var$, eps$] apply the inference-form batch-norm y = gamma$ * (x$ - mean$) / sqrt(var$ + eps$) + beta$ along the channel axis. x$ is rank-3 {C, H, W} or rank-4 {B, C, H, W}; gamma$, beta$, mean$, var$ are all rank-1 {C}. Default eps$ = 1e-5."];
SetUsage[TBatchNormTrain, "TBatchNormTrain[x$, gamma$, beta$] and TBatchNormTrain[x$, gamma$, beta$, eps$] compute batch-norm using statistics from x$, then apply per-channel gamma$ and beta$. Supports rank-3 {C, H, W} and rank-4 {B, C, H, W}."];
...
TLayerNormAffine[x_TTerm, gamma_TTerm, beta_TTerm] := With[{shape = tUopShape[x], norm = TLayerNorm[x]},
...
TGroupNorm[x_TTerm, weight_TTerm, bias_TTerm, groups_Integer, eps_ ? NumericQ] := With[{shape = tUopShape[x]},
...
TBatchNorm[x_TTerm, gamma_TTerm, beta_TTerm, mean_TTerm, var_TTerm] := TBatchNorm[x, gamma, beta, mean, var, 1.0*^-5]
TBatchNorm[x_TTerm, gamma_TTerm, beta_TTerm, mean_TTerm, var_TTerm, eps_ ? NumericQ] := With[{shape = tUopShape[x]},
    Module[{rank, b, c, h, w, scaleC, shiftC, bcastShape, scaleBcast, shiftBcast},
        ...
        If[ rank === 3,
            ...
            If[ rank === 4,
                ...
                bcastShape = {1, c, 1, 1}
                ,
                Return @ Failure["NotImplemented", <|"Message" -> "TBatchNorm expects rank-3 or rank-4 input", "InputShape" -> shape|>]
            ]
        ];
...
TBatchNormTrain[x_TTerm, gamma_TTerm, beta_TTerm] := TBatchNormTrain[x, gamma, beta, 1.0*^-5]
TBatchNormTrain[x_TTerm, gamma_TTerm, beta_TTerm, eps_ ? NumericQ] := With[{shape = tUopShape[x]},
    Module[{rank, b, c, h, w, reduceCount, bcastShape, mean, centered, var, broadcast},
        rank = Length[shape];
        If[ rank === 3,
            c = shape[[1]];
            h = shape[[2]];
            ...
            If[ rank === 4,
                ...
                reduceCount = b * h * w;
                bcastShape = {1, c, 1, 1};
                mean = TUOpReduce[TUOpReduce[TUOpReduce[x, 3, "SUM"], 2, "SUM"], 0, "SUM"] / reduceCount
                ,
                Return @ Failure["NotImplemented", <|"Message" -> "TBatchNormTrain expects rank-3 or rank-4 input", "InputShape" -> shape|>]
            ]
        ];
        broadcast[t_] := TUOpExpand[TUOpReshape[t, bcastShape], shape];
        centered = x - broadcast[mean];
        var = If[ rank === 3,
            TUOpReduce[TUOpReduce[centered * centered, 2, "SUM"], 1, "SUM"] / reduceCount,
            TUOpReduce[TUOpReduce[TUOpReduce[centered * centered, 3, "SUM"], 2, "SUM"], 0, "SUM"] / reduceCount
        ];
        centered / Sqrt[broadcast[var] + eps] * broadcast[gamma] + broadcast[beta]
    ]
]
