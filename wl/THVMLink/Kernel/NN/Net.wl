...
SetUsage[TFromNet, "TFromNet[net$, x$] converts a Wolfram NeuralNetworks layer (or NetChain / NetGraph) into a TTerm UOp graph rooted at the input TTerm x$; net$ must be initialised so its weights are concrete arrays.
...
SetUsage[TToNet, "TToNet[lam$] best-effort reconstructs a NetChain from the graph of a TFromNet[net]-built LAM (no net is stored). It recognises the standard layer sub-DAG signatures (Linear / Elementwise ReLU / Softmax / Flatten), reading each layer's weights off the graph leaves, and infers the input dimension from the first Linear weight. Returns a Failure for a body with a conv / pool / unrecognised op, whose im2col lowering does not round-trip cleanly."];
SetUsage[TNetParams, "TNetParams[lam$] returns the trainable weight handles (float-leaf TTerms) of a TFromNet[net] LAM, in the C leaf-walk's stable order. The LAM's bound input is a hole and is excluded, leaving exactly the weights baked into the forward; TGrad differentiates every reachable float leaf and TNetTrain updates them in place. As in tinygrad's get_parameters, an applied forward TFromNet[net, x] has a concrete input baked in and no distinguished parameters, so lift with TFromNet[net] instead."];
SetUsage[TNetParamInfo, "TNetParamInfo[lam$] returns the trainable weight handles of a TFromNet[net] LAM paired with best-effort provenance, as a list of <|\"Term\", \"Layer\", \"Param\"|> associations. Without a stored net the Layer is Missing[\"NotStored\"] and Param is inferred from the handle's shape (rank-2+ gives \"Weights\", rank-1 gives \"Biases\"); TNetParams[lam$] returns the same handles without provenance."];
SetUsage[TNetInitialize, "TNetInitialize[lam$] re-initialises the trainable weights of a TFromNet[net] LAM in place: Glorot for weight matrices, ones for normalisation scalings, zeros for biases. Installed as the NetInitialize UpValue on TTerms."];
SetUsage[TFromLayer, "TFromLayer[layer$, x$] is the single-layer form of TFromNet."];
SetUsage[TLayerWeights, "TLayerWeights[layer$] returns the NumericArrays of every learnable parameter layer$ carries (Weights, Biases, $$), in the layer-specific declaration order."];
SetUsage[TLayerToTensors, "TLayerToTensors[layer$] is like TLayerWeights but wraps each NumericArray in a fresh TTensorCreate handle so it can feed back into a UOp graph."];
Begin["`Private`"];
...
TLayerWeights[layer_] := NetExtract[layer, #] & /@ $layerParams[Head[layer]]
...
asRowVec[x_TTerm] := With[{shape = tUopShape[x]},
    If[Length[shape] === 1, TUOpReshape[x, {1, shape[[1]]}], x]
]
...
fromLayer[LinearLayer, layer_, x_TTerm] := Module[{wArr, bArr, w},
    ...
    If[ bArr === None,
        If[Length[tUopShape[x]] <= 1, TMatVec[w, asRowVec[x]], TMatMul[x, Transpose[w]]]
        ,
        With[{b = TTensorCreate[bArr]},
            If[Length[tUopShape[x]] <= 1, TUOpAdd[TMatVec[w, asRowVec[x]], b], TLinear[x, Transpose[w], b]]
        ]
    ]
]
...
$elementwiseDispatch = <|Identity -> TIdentity, (#1 #1 &) -> TSquare, (-#1 &) -> TUOpNeg, Ramp -> TReLU, Tanh -> TTanh|>
...
fromLayer[ReshapeLayer, layer_, x_TTerm] := With[{out = NetExtract[layer, "Output"]},
...
fromLayer[FlattenLayer, _, x_TTerm] := With[{shape = tUopShape[x]},
    If[ Length[shape] >= 4,
...
fromLayer[PoolingLayer, layer_, x_TTerm] := Module[{kSize, stride, fn, shape, h, w, kh, kw},
    fn = NetExtract[layer, "Function"];
    kSize = NetExtract[layer, "KernelSize"];
    stride = NetExtract[layer, "Stride"];
    If[ fn =!= Max,
        Return @ Failure["NotImplemented", <|"Message" -> "PoolingLayer Function != Max not yet supported", "fn" -> fn|>]];
    If[ kSize =!= stride,
        Return @ Failure["NotImplemented", <|"Message" -> "PoolingLayer overlapping (Stride != KernelSize) not yet supported", "KernelSize" -> kSize, "Stride" -> stride|>]];
    shape = tUopShape[x];
    If[ ! MemberQ[{3, 4}, Length[shape]],
        Return @ Failure["NotImplemented", <|"Message" -> "PoolingLayer expects rank-3 {C, H, W} or rank-4 {B, C, H, W} input", "InputShape" -> shape|>]];
    {h, w} = shape[[-2 ;;]];
    {kh, kw} = kSize;
    If[ Mod[h, kh] =!= 0 || Mod[w, kw] =!= 0,
        Return @ Failure["NotImplemented", <|"Message" -> "PoolingLayer non-overlap requires H, W divisible by kernel", "InputShape" -> shape, "KernelSize" -> kSize|>]];
    If[ kh =!= kw,
        Return @ Failure["NotImplemented", <|"Message" -> "PoolingLayer non-square kernel sizes need a separate path", "KernelSize" -> kSize|>]];
    TMaxPool2d[x, kh]
]
...
fromLayer[EmbeddingLayer, _, _TTerm] := Failure["NotImplemented", <|"Message" -> "EmbeddingLayer with TTerm input requires UOP_GATHER (Phase 12+).  Pass List[Integer] ids on the host side instead."|>]
...
fromLayer[ThreadingLayer, layer_, xs_List] := Module[{f, isPlus, isTimes},
    ...
    Which[
        ...
        isPlus, Total[xs],
        isTimes, Times @@ xs,
        True, Failure["NotImplemented", <|"Message" -> "ThreadingLayer Function not supported", "Function" -> f|>]
    ]
]
...
linearTransposedTensor[layer_LinearLayer] := With[{key = layer},
    If[ KeyExistsQ[$linearTransposedCache, key],
        ...
        $linearTransposedCache[key] = TTensorCreate @ NumericArray[Transpose @ Normal @ NetExtract[layer, "Weights"], "Real32"]
    ]
]
...
fromLayer[CatenateLayer, layer_, xs_List] := Module[{axis, rank, widths, offsets, total},
    ...
    total = Total[widths];
    Total @ MapThread[{t, off, w} |-> TUOpPad[t, Table[If[a === axis, {off, total - off - w}, {0, 0}], {a, rank}]], {xs, offsets, widths}]
]
...
(* SequenceIndicesLayer: produces a sequence of integer position
   ...
   is just `Range[1, len]`. *)
fromLayer[SequenceIndicesLayer, _, ids_List] /; AllTrue[ids, IntegerQ] := Range[1, Length[ids]]
fromLayer[SequenceIndicesLayer, _, _] := Failure["NotImplemented", <|"Message" -> "SequenceIndicesLayer needs host-side List[Integer] input"|>]
...
mhaHeadQ[h_] := MatchQ[h, _NetGraph] && MemberQ[Head /@ mhaGraphNodes[h], AttentionLayer]

mhaSelfAttnGraphQ[g_] := MatchQ[g, _NetGraph] && Module[{ns = mhaGraphNodes[g]},
    Length[ns] >= 3 && MatchQ[Last[ns], _NetMapOperator] && MatchQ[ns[[-2]], _CatenateLayer] && AllTrue[ns[[1 ;; -3]], mhaHeadQ]
]

fromLayer[NetGraph, g_, x_TTerm] /; mhaSelfAttnGraphQ[g] := Module[{ns, heads, nH, dim, dH, stackW, stackB, Wq, bq, Wk, bk, Wv, bv, outProj, Wo, bo, scaleFn, scale, maskSpec, lin, q, k, v, attn, seq, b, kC, vC},
    ns = mhaGraphNodes[g];
    heads = ns[[1 ;; -3]];
    outProj = Last[ns];
    nH = Length[heads];
    ...
    maskSpec = Quiet @ Lookup[NetExtract[First[heads], {"attention", "Parameters"}], "Mask", None];
    ...
    attn = If[ AssociationQ[$decodeAttn],
        ...
        TDecodeAttend[q, kC, vC, k, v, nH, $decodeAttn["posVid"], $decodeAttn["lenVid"], scale]
        ,
        seq = First @ tUopShape[q];
        TMultiHeadAttention[q, k, v, nH, If[MatchQ[maskSpec, "Causal" | Causal], If[symLeadingQ[q], TCausalMaskSym[symVid[seq], TKVarHi[symVid[seq]]], TCausalMask[seq]], None], scale]
    ];
    lin[attn, Wo, bo]
]
...
fromLayer[NetGraph, g_, input_] := Module[{childAssoc, topNodes, lg, rawEdges, projectTop, topEdges, sortGraph, sorted, results, predsOf, getInput},
    ...
    projectTop[path_List] := SelectFirst[path, MemberQ[topNodes, #] &, Missing[]];

    lg = Information[g, "LayersGraph"];
    rawEdges = EdgeList[lg];
    topEdges = DeleteDuplicates @ DeleteCases[Cases[rawEdges, DirectedEdge[s_List, d_List] :> {projectTop[s], projectTop[d]}], {a_, a_} | {_Missing, _} | {_, _Missing}];
    ...
    getInput[n_] := Module[{node = childAssoc[n], preds = predsOf[n], arity, predTerms, distinct},
        arity = Length @ Replace[Information[node, "InputPortNames"], _Missing -> {Input}];
        ...
        Which[
            ...
            arity > 1 && Length[distinct] <= 1 && MatchQ[Head[node], NetGraph | NetMapOperator | NetChain],
    ...
    Do[results[n] = TFromLayer[childAssoc[n], getInput[n]], {n, sorted}];
    results[Last[sorted]]
]
...
fromLayer[AttentionLayer, layer_, qkv_List] /; Length[qkv] === 3 && AssociationQ[$decodeAttn] := Module[{q, k, v, params, mask, rescale, nHeads, dim, scale, qShape, portNames, qi, ki, vi, b, kCache, vCache},
    portNames = Replace[Information[layer, "InputPortNames"], Except[_List] -> {"Query", "Key", "Value"}];
    ...
    TDecodeAttend[q, kCache, vCache, k, v, nHeads, $decodeAttn["posVid"], $decodeAttn["lenVid"], scale]
]

fromLayer[AttentionLayer, layer_, qkv_List] /; Length[qkv] === 3 := Module[{q, k, v, params, mask, rescale, qShape, nHeads, seq, dim, scale, portNames, qi, ki, vi},
    ...
    scale = Switch[ rescale,
        ...
        "DimensionSqrt", 1 / Sqrt[N[dim / nHeads]],
        _, 1 / Sqrt[N[dim / nHeads]]];
    TMultiHeadAttention[q, k, v, nHeads, If[MatchQ[mask, "Causal" | Causal], If[symLeadingQ[q], TCausalMaskSym[symVid[seq], TKVarHi[symVid[seq]]], TCausalMask[seq]], None], scale]
]
fromLayer[AttentionLayer, _, _] := Failure["NotImplemented", <|"Message" -> "AttentionLayer expects {Q, K, V} TTerm triple via NetGraph multi-input dispatch"|>]
...
fromLayer[ConvolutionLayer, layer_, x_TTerm] := Module[{w, b, stride, pad, dil},
    stride = NetExtract[layer, "Stride"];
    pad = NetExtract[layer, "PaddingSize"];
    dil = NetExtract[layer, "Dilation"];
    If[ stride =!= {1, 1},
        Return @ Failure["NotImplemented", <|"Message" -> "ConvolutionLayer Stride != {1,1} not yet supported", "Stride" -> stride|>]];
    If[ pad =!= {{0,0},{0,0}} && pad =!= 0,
        Return @ Failure["NotImplemented", <|"Message" -> "ConvolutionLayer PaddingSize != 0 not yet supported", "PaddingSize" -> pad|>]];
    If[ dil =!= {1, 1},
        Return @ Failure["NotImplemented", <|"Message" -> "ConvolutionLayer Dilation != {1,1} not yet supported", "Dilation" -> dil|>]];
...
TFromNet[chain_NetChain, x_TTerm] := Fold[TFromLayer[#2, #1] &, x, Table[chain[[i]], {i, Length[chain]}]]
...
TFromNet[chain_NetChain, ids_List] := Module[{hidden, tokEmb},
    hidden = Fold[TFromLayer[#2, #1] &, ids, Table[chain[[i]], {i, Length[chain]}]];
    tokEmb = tokenEmbeddingArray[chain];
    If[ MatchQ[hidden, _TTerm] && tokEmb =!= None && Length[tUopShape[hidden]] === 2 && Last[tUopShape[hidden]] === Last[Dimensions[tokEmb]],
...
allLayers[net_] := If[ MatchQ[Head[net], NetChain | NetGraph | NetMapOperator],
...
tokenEmbeddingArray[net_] := Module[{embs, weights},
    ...
    weights = Cases[Quiet @ NetExtract[#, "Weights"] & /@ embs, w_ /; MatchQ[Dimensions[w], {_, _}]];
    If[ weights === {}, None,
...
netInputShape[net_] := Module[{ports, raw, shape},
    ...
    If[ MatchQ[net, _NetChain] && Length[net] >= 1,
        With[{l1Ports = Quiet @ Information[NetExtract[net, 1], "InputPorts"]},
            If[AssociationQ[l1Ports] && Length[l1Ports] === 1, Return[shapeFromPort[First[Values[l1Ports]]]]]
        ]
    ];
    $Failed
]
...
tokenLmForward[net_, onehot_, maxSeq_Integer] := Module[{tokTable, posTable, dim, restLayers, outDims, needsHead, tokT, posT, x, hidden},
    ...
    restLayers = DeleteCases[Drop[Table[net[[i]], {i, Length[net]}], 1], _SequenceLastLayer];
    ...
    posT = If[ symLeadingQ[onehot],
        With[{vid = symVid[First @ tUopShape[onehot]]},
            ...
            TUOpCopy[TSymbolicAxis[TTensorCreateHost[posTable[[1 ;; TKVarHi[vid]]]], 0, vid]]
        ]
        ,
...
(* tokenLmDecodeStep[net, oneHotRow, kCaches, vCaches, posVid, lenVid] -- the
   ...
   position. *)
tokenLmDecodeStep[net_, oneHotRow_TTerm, kCaches_List, vCaches_List, posVid_Integer, lenVid_Integer] := tokenLmDecodeStep[net, oneHotRow, kCaches, vCaches, posVid, lenVid, Automatic]
...
tokenLmDecodeStep[net_, oneHotRow_TTerm, kCaches_List, vCaches_List, posVid_Integer, lenVid_Integer, posRow_] := Module[{tokTable, posTable, dim, restLayers, outDims, needsHead, tokT, posT, posLeaf, x, hidden},
    tokTable = tokenEmbeddingArray[net];
    posTable = positionEmbeddingArray[net];
    dim = Last[Dimensions[tokTable]];
    restLayers = DeleteCases[Drop[Table[net[[i]], {i, Length[net]}], 1], _SequenceLastLayer];
    ...
    posT = If[ posRow === Automatic,
        ...
        TUOpShrink[posLeaf, {{TKVarPack[posVid], TKVarPack[posVid] + 1}, {0, dim}}]
        ,
        posRow
    ];
    ...
    hidden = Block[{$decodeAttn = <|"k" -> kCaches, "v" -> vCaches, "posVid" -> posVid, "lenVid" -> lenVid, "idx" -> 1|>},
...
TDecodeStep[net_, oneHotRow_TTerm, state_Association] := tokenLmDecodeStep[net, oneHotRow, state["kCaches"], state["vCaches"], state["posVid"], state["lenVid"]]
...
TDecodeStep[net_, oneHotRow_TTerm, posRow_TTerm, state_Association] := tokenLmDecodeStep[net, oneHotRow, state["kCaches"], state["vCaches"], state["posVid"], state["lenVid"], posRow]
...
decodePosRow[state_Association, pos_Integer] := TTensorCreate[N @ {Normal @ state["posTable"][[pos + 1]]}]
...
TDecodeNext[net_, state_Association, tokenId_Integer] := Module[{oneHotRow, posRow, fn, logits, st},
    ...
    fn = state["fn"];
    If[fn === None, fn = TJit[{oh, pr} |-> TRealize @ TDecodeStep[net, oh, pr, state]]];
    st = If[state["fn"] === None, <|state, "fn" -> fn|>, state];
    logits = First @ Normal @ TTensorData @ fn[oneHotRow, posRow];
    <|st, "pos" -> st["pos"] + 1, "token" -> First[Ordering[logits, -1]] - 1, "logits" -> logits|>
]
...
(* TFromNet[net, oneHot]: the fixed-sequence forward APPLIED to a {maxSeq,
   ...
   more specific). *)
TFromNet[net_NetChain, oneHot_TTerm] /; tokenLmOneHotQ[net, oneHot] := tokenLmForward[net, oneHot, First[tUopShape[oneHot]]]
...
TFromNet[net_NetChain] /; tokenEmbeddingArray[net] =!= None := oneHot |-> TFromNet[net, oneHot]
...
paramInfoOf[handles_List] := (t |-> <|"Term" -> t, "Layer" -> Missing["NotStored"], "Param" -> paramKindFromShape[TTensorShape[t]]|>) /@ handles
...
initParams[handles_List] := (
    (t |-> With[{shape = TTensorShape[t]}, TSet[t, If[Length[shape] >= 2, TGlorot[shape], TZeros[shape]]]]) /@ handles;
)
...
unwrapMove[t_] := If[ uTag[t] === $TagUOP && MemberQ[{$UopReshape, $UopExpand, $UopPermute}, uExt[t]],
...
peelLayer[t_] := Module[{red, mul, wMove, bMove, w, bias, wArr, inL, inR, inner},
    Which[
        (* LinearLayer: ADD[REDUCE[MUL[EXPAND RESHAPE in, EXPAND RESHAPE PERMUTE W]],
                             EXPAND RESHAPE bias] *)
        uIsUop[t, $UopAdd] && uIsUop[(red = uChild[t, 0]), $UopReduce] && uIsUop[(mul = uChild[red, 0]), $UopMul] && uIsTen[(bMove = unwrapMove[uChild[t, 1]])],
            ...
            wArr = tenArray[w];
            {LinearLayer[First[Dimensions[wArr]], "Weights" -> wArr, "Biases" -> tenArray[bias]], inner}
        ,
        (* ReLU: MUL[CMPLT[EXPAND CONST(0), inner], inner] *)
        uIsUop[t, $UopMul] && uIsUop[uChild[t, 0], $UopCmplt],
            {ElementwiseLayer[Ramp], unwrapMove[uChild[t, 1]]}
        ,
        (* SoftmaxLayer: TSoftmax root = MUL[exp(x-max), EXPAND RECIP sum] *)
        uIsUop[t, $UopMul] && uIsUop[uChild[t, 1], $UopExpand] && uIsUop[uChild[uChild[t, 1], 0], $UopRecip],
...
TToNet[lam_TTerm] := Module[{body, layers, t, step, guard},
    ...
    While[ uTag[t] =!= $TagVAR && guard < 256,
        ...
        step = peelLayer[t];
        If[ step === $Failed,
            Return @ Failure["TToNetUnsupported", <|"Message" -> "TToNet: graph layer not reconstructible " <> "(conv / pool / unrecognised op); best-effort recovery " <> "supports Linear / Elementwise / Softmax / Flatten only."|>]];
        AppendTo[layers, First[step]];
        t = Last[step]
    ];
    If[ uTag[t] =!= $TagVAR,
        Return @ Failure["TToNetUnsupported", <|"Message" -> "TToNet: did not reach the input variable."|>]];
