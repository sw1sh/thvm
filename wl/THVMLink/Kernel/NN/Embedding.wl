...
SetUsage[TGather, "TGather[x$, axis$, index$] selects one element per slice along axis$ (0-indexed): out[$$, 0, $$] = x$[$$, index$, $$], keeping axis$ at size 1. It lowers tinygrad-style to sum((arange == index$) * x$) over axis$ -- the CMPEQ one-hot the cross-entropy uses, multiplied by x$ and sum-reduced. index$ is an integer TTerm or host list giving one index per slice (x$'s shape with axis$ -> 1, or the flat per-slice list). The gradient scatters the cotangent back to the selected positions, so it trains. See TTakeAlongAxis for the numpy (index$, axis$) order."];
SetUsage[TTakeAlongAxis, "TTakeAlongAxis[x$, index$, axis$] is TGather[x$, axis$, index$] with the numpy take_along_axis argument order: select x$ at integer index$ along axis$, keeping the axis at size 1. This is the per-sample selection a policy gradient uses to pick each action's log-prob from {batch, n_actions} logits$ with a {batch} action list (axis$ = 1)."];
SetUsage[TEmbedding, "TEmbedding[table$, idx$] returns row idx$ of a {V, D} table$ as a TTerm of shape {D}. idx$ is a host-side integer; it lowers to a shrink + reshape to {D}. Dynamic-idx gather (idx$ as a runtime tensor) needs a future gather opcode."];
SetUsage[TEmbeddingMatrix, "TEmbeddingMatrix[table$, ids$] gathers rows ids$ from a {V, D} table$ into a {Length[ids$], D} matrix. ids$ is a host-side list of integers. It lowers to one TEmbedding + reshape to {1, D} per id, then stitches along the leading axis via pad + sum (thvm has no stack op). Dynamic-id gather needs a future gather opcode."];
SetUsage[TSinusoidalEmbedding, "TSinusoidalEmbedding[t$, dim$] returns the {dim$} sinusoidal timestep / position embedding of scalar t$ in the diffusers cos-first order (flip_sin_to_cos): concat[cos(t$ * freqs), sin(t$ * freqs)] with freqs[i] = exp(-log(10000) * i / (dim$/2)). The weight-free part of a diffusion-model time embedder. t$ is a host number (or list), dim$ the embedding width."];
...
TGather[x_TTerm, axis_Integer, index_] := Module[{shape = tUopShape[x], rank, n, keepShape, arange, idx, oh},
    rank = Length[shape];
    n = shape[[axis + 1]];
    keepShape = ReplacePart[shape, axis + 1 -> 1];
    arange = TUOpExpand[TUOpReshape[TTensorCreate[N @ Range[0, n - 1]], ReplacePart[ConstantArray[1, rank], axis + 1 -> n]], shape];
...
TEmbedding[table_TTerm, idx_Integer] := With[{shape = tUopShape[table]},
    Module[{d},
        d = shape[[2]];
        TUOpReshape[TUOpShrink[table, {{idx, idx + 1}, {0, d}}], {d}]
    ]
]

TEmbeddingMatrix[table_TTerm, ids_List] := With[{shape = tUopShape[table]},
    Module[{d, len, rows, padded},
        d = shape[[2]];
        len = Length[ids];
        rows = TUOpReshape[TEmbedding[table, #], {1, d}] & /@ ids;
        padded = MapIndexed[TUOpPad[#1, {{First[#2] - 1, len - First[#2]}, {0, 0}}] &, rows];
        Total[padded]
    ]
]
