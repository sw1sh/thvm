...
GeneralUtilities`SetUsage[TSiLU, "TSiLU[x$] = x$ * TSigmoid[x$], the SiLU / swish activation (the SwiGLU MLP gate). Mirrors tinygrad's Tensor.silu."];
GeneralUtilities`SetUsage[TSwiGLU, "TSwiGLU[h$] is the SwiGLU activation on a {$$, 2*m} tensor: split the last axis into halves x1$ | x2$ and return TSiLU[x1$] * x2$, a {$$, m} tensor (the Flux2/LLaMA gated MLP activation).
...
TSwiGLU[x_TTerm, wIn_TTerm, wOut_TTerm] :=
    TSwiGLU[x . Transpose[wIn]] . Transpose[wOut]
