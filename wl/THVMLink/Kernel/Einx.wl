...
SetUsage[TEinRearrange, "TEinRearrange[pattern$, x$, hints$$] reshapes, permutes, and broadcasts x$ by the einx pattern$ (which must specify \[Rule]).
...
SetUsage[TEinSum, "TEinSum[pattern$, x$, hints$$] sums over the bracketed axes in the einx pattern$.
...
SetUsage[TEinMean, "TEinMean[pattern$, x$, hints$$] averages over the bracketed axes."];
SetUsage[TEinMax, "TEinMax[pattern$, x$, hints$$] reduces (max) over the bracketed axes."];
SetUsage[TEinMin, "TEinMin[pattern$, x$, hints$$] reduces (min) over the bracketed axes."];
SetUsage[TEinProd, "TEinProd[pattern$, x$, hints$$] reduces (product) over the bracketed axes; inputs must be strictly positive."];
SetUsage[TEinAny, "TEinAny[pattern$, x$, hints$$] logical OR over the bracketed axes of a 0/1-valued input."];
SetUsage[TEinAll, "TEinAll[pattern$, x$, hints$$] logical AND over the bracketed axes of a 0/1-valued input."];
SetUsage[TEinVar, "TEinVar[pattern$, x$, hints$$] variance over the bracketed axes."];
SetUsage[TEinStd, "TEinStd[pattern$, x$, hints$$] standard deviation over the bracketed axes."];
SetUsage[TEinLogSumExp, "TEinLogSumExp[pattern$, x$, hints$$] numerically-stable log-sum-exp over the bracketed axes."];

SetUsage[TEinSoftmax, "TEinSoftmax[pattern$, x$, hints$$] numerically-stable softmax along the bracketed axis, returning the input shape."];
SetUsage[TEinLayerNorm, "TEinLayerNorm[pattern$, x$, hints$$] normalizes x$ to mean 0, variance 1 over the bracketed axes (default eps 1.0e-5).
TEinLayerNorm[pattern$, x$, eps$, hints$$] sets the epsilon added under the square root."];
SetUsage[TEinRMSNorm, "TEinRMSNorm[pattern$, x$, hints$$] divides x$ by its root-mean-square over the bracketed axes (default eps 1.0e-5).
TEinRMSNorm[pattern$, x$, eps$, hints$$] sets the epsilon added under the square root."];

SetUsage[TEinAdd, "TEinAdd[pattern$, args$$, hints$$] broadcasting elementwise add, e.g. pattern \"b s d, d \[Rule] b s d\"."];
SetUsage[TEinMul, "TEinMul[pattern$, args$$, hints$$] broadcasting elementwise multiply."];
SetUsage[TEinSub, "TEinSub[pattern$, args$$, hints$$] broadcasting elementwise subtract."];
SetUsage[TEinDiv, "TEinDiv[pattern$, args$$, hints$$] broadcasting elementwise divide."];
SetUsage[TEinEq, "TEinEq[pattern$, args$$, hints$$] broadcasting elementwise equality (1.0 where equal, else 0.0)."];
SetUsage[TEinLt, "TEinLt[pattern$, args$$, hints$$] broadcasting elementwise less-than (1.0 where less, else 0.0)."];
SetUsage[TEinWhere, "TEinWhere[pattern$, cond$, a$, b$, hints$$] broadcasting elementwise select: cond$ a$ + (1 - cond$) b$."];

SetUsage[TEinDot, "TEinDot[pattern$, x$, y$, hints$$] contracts x$ and y$ by the einx pattern$ (which must specify \[Rule]).
Axes appearing in the inputs but not the output are summed; a rank-2 collapsed contraction dispatches through TMatMul."];

SetUsage[TEinFlip, "TEinFlip[pattern$, x$, hints$$] flips the bracketed axes, e.g. TEinFlip[\"b [s] d\", x$]."];
SetUsage[TEinRoll, "TEinRoll[pattern$, x$, shift$, hints$$] circularly shifts x$ along the single bracketed axis by shift$ positions, lowered via Shrink, Pad, and sum."];

SetUsage[TEinGetAt, "TEinGetAt[pattern$, x$, idx$, hints$$] gathers rows of x$ at the host-side integer list idx$, e.g. TEinGetAt[\"[v] d, b \[Rule] b d\", table$, {1, 7, 3}].
A dynamic-tensor idx$ is not yet supported (needs a UOP_GATHER opcode); see the TEmbedding note in NN.wl."];
SetUsage[TEinSetAt, "TEinSetAt[pattern$, x$, idx$, vals$, hints$$] scatters rows of vals$ into x$ at the host-side integer positions idx$ (distinct), returning a fresh TTerm and leaving x$ unchanged."];
SetUsage[TEinAddAt, "TEinAddAt[pattern$, x$, idx$, vals$, hints$$] scatter-adds rows of vals$ into x$ at the host-side integer positions idx$."];
SetUsage[TEinSubAt, "TEinSubAt[pattern$, x$, idx$, vals$, hints$$] scatter-subtracts rows of vals$ from x$ at the host-side integer positions idx$."];
...
einxParseFail[reason_String, ctx___] := Throw @ Failure["EinxParse", <|"Reason" -> reason, ctx|>]
...
einxFoldStep[state_, tok_] := Block[{slots = state["slots"], bracketed = state["bracketed"], mode = state["mode"], cur = state["cur"]},
    Switch[mode
        ,
        "top",
            Switch[tok,
        ...
        "bracket",
            Switch[tok,
                ...
                _String,
                    <|state, "slots" -> Append[slots, tok], "bracketed" -> Append[bracketed, tok], "mode" -> "brkEnd"|>,
                _, einxParseFail["expected ident or ( after [", "Token" -> tok]
            ]
        ,
        "brkEnd",
            If[tok === "]", <|state, "mode" -> "top"|>, einxParseFail["expected ]", "Token" -> tok]]
        ,
        "group",
            Switch[tok,
                ")", <|state, "slots" -> Append[slots, cur], "cur" -> Null, "mode" -> "top"|>,
        ...
        "brkGroup",
            Switch[tok,
                ")", <|state, "slots" -> Append[slots, cur], "bracketed" -> Join[bracketed, Cases[cur, _String]], "cur" -> Null, "mode" -> "brkEnd"|>,
                _String | _Integer, <|state, "cur" -> Append[cur, tok]|>,
                _, einxParseFail["bad token in [(...)]", "Token" -> tok]
            ]
    ]
...
einxParseSide[str_String] := Block[{toks = einxTokenize[str], init, final},
...
parseEinxPattern[str_String] := Catch @ Block[{parts, lhs, rhs, lhsBits, inPairs, outSlots, outBracketed, outSpecified},
...
einxResolveSizes[parsed_, args_List, hints_Association] := Block[{sizes = hints, inputSlots = parsed["in"], inputShapes, advanced, steps},
    inputShapes = tUopShape /@ args;
    MapThread[
        Function[{slots, shape, idx},
            If[ Length[slots] =!= Length[shape],
                einxParseFail["input rank mismatch", "Input" -> idx, "Slots" -> slots, "Shape" -> shape]];
            MapThread[
                Function[{slot, dim},
                    Which[
                        IntegerQ[slot],
                            If[ slot =!= dim,
                                einxParseFail["literal-axis size mismatch", "Input" -> idx, "Expected" -> slot, "Actual" -> dim]],
                        StringQ[slot],
                        ...
                        If[ ListQ[slot],
                            Block[{names, known, unknown, prodKnown},
                                ...
                                Which[
                                    Length[unknown] === 0,
                                        If[ prodKnown =!= dim,
                                            einxParseFail["composite product mismatch", "Group" -> slot, "Product" -> prodKnown, "Actual" -> dim]],
                                    Length[unknown] === 1,
                                        If[ Mod[dim, prodKnown] =!= 0,
                                            einxParseFail["composite not divisible", "Group" -> slot, "Dim" -> dim, "Known" -> prodKnown]];
...
einxDefaultOut[parsed_, verbClass_] := Block[{inFlatNames = DeleteDuplicates @ Catenate[ flatNames /@ parsed["in"] ], bracketed = parsed["bracketed"]},
    Switch[verbClass
        ,
        "reduce",
...
einxOutFlatNames[parsed_, verbClass_] := If[parsed["outSpecified"], flatNames @ parsed["out"], einxDefaultOut[parsed, verbClass]]
...
einxReduceAxes["dot", parsed_, outNames_] := DeleteDuplicates @ Complement[Catenate[ flatNames /@ parsed["in"] ], outNames]
...
(* Stable ordering used throughout: out-names first (preserves the
   ...
   the inputs. *)
einxGlobalAxes[parsed_, outNames_, reduceNames_] := DeleteDuplicates @ Join[outNames, reduceNames, Catenate[ flatNames /@ parsed["in"] ]]
...
*)
einxLiftInput[arg_TTerm, slots_List, sizes_, globalAxes_] := Block[{flatAxes, stringAxes, splitShape, splitTerm, droppedTerm, presentSizes, permOrder, permuted, unitsShape, unitsTerm, fullShape, expanded},
    ...
    droppedTerm = If[Length[stringAxes] === Length[flatAxes], splitTerm, TUOpReshape[splitTerm, presentSizes]];

    permOrder = SortBy[Range[Length[stringAxes]], Position[globalAxes, stringAxes[[#]]][[1, 1]] &];
    permuted = If[permOrder === Range[Length[stringAxes]], droppedTerm, TUOpPermute[droppedTerm, permOrder - 1]];

    unitsShape = Map[If[MemberQ[stringAxes, #], sizes[#], 1] &, globalAxes];
    unitsTerm = If[Length[unitsShape] === Length[stringAxes], permuted, TUOpReshape[permuted, unitsShape]];

    fullShape = sizes /@ globalAxes;
    expanded = If[unitsShape === fullShape, unitsTerm, TUOpExpand[unitsTerm, fullShape]];
    expanded
]
...
einxReduceAlong[term_, axes_, name_String, kind_String] := Block[{pos = Position[axes, name][[1, 1]]},
...
*)
einxFinalize[term_, curAxes_, parsed_, outFlatNames_, sizes_] := Block[{permOrder, permuted, finalShape, slots = parsed["out"]},
    If[ Sort[curAxes] =!= Sort[outFlatNames],
        einxParseFail["finalize axis mismatch", "Have" -> curAxes, "Want" -> outFlatNames]
    ];
    permOrder = SortBy[Range[Length[curAxes]], Position[outFlatNames, curAxes[[#]]][[1, 1]] &];
    permuted = If[permOrder === Range[Length[curAxes]], term, TUOpPermute[term, permOrder - 1]];
    ...
    If[! parsed["outSpecified"] || finalShape === (sizes /@ outFlatNames), permuted, TUOpReshape[permuted, finalShape]]
]
...
einxPrepare[parsed_, args_List, hints_Association, verbClass_] := Block[{sizes, outNames, reduceNames, globalAxes, lifted},
    ...
    Scan[
        Function[n,
            If[ ! KeyExistsQ[sizes, n],
                einxParseFail["axis appears only in output (add a size hint)", "Axis" -> n]]
        ],
        Complement[globalAxes, Catenate[ flatNames /@ parsed["in"] ]]
    ];
    lifted = MapThread[einxLiftInput[#1, #2, sizes, globalAxes] &, {args, parsed["in"]}];
    <|
        "sizes" -> sizes,
        "outNames" -> outNames,
...
einxSplitArgs[args_List] := <|"tensors" -> Cases[args, Except[_Rule]], "hints" -> Association @ Cases[args, _Rule]|>

(* === movement verbs ============================================== *)

einxMovement[parsed_, prep_, op_:Identity] := Block[{term = First[prep["lifted"]], curAxes = prep["globalAxes"]},
    einxFinalize[op[term], curAxes, parsed, prep["outNames"], prep["sizes"]]
]

TEinRearrange[pattern_String, x_TTerm, rest___] := Catch @ Block[{parsed, split, prep},
    ...
    If[ split["tensors"] =!= {},
        einxParseFail["TEinRearrange takes one tensor", "Extra" -> split["tensors"]]];
...
TEinFlip[pattern_String, x_TTerm, rest___] := Catch @ Block[{parsed, split, prep, flipAxisIds, term, finalParsed},
    parsed = parseEinxPattern[pattern];
    split = einxSplitArgs[{rest}];
    prep = einxPrepare[parsed, {x}, split["hints"], "elem"];
    flipAxisIds = (Position[prep["globalAxes"], #][[1, 1]] - 1) & /@ parsed["bracketed"];
    term = If[flipAxisIds === {}, First @ prep["lifted"], TUOpFlip[First @ prep["lifted"], flipAxisIds]];
    (* For TEinFlip the "output" defaults to the LHS unchanged. *)
    finalParsed = If[parsed["outSpecified"], parsed, <|parsed, "out" -> First[parsed["in"]], "outSpecified" -> True|>];
    einxFinalize[term, prep["globalAxes"], finalParsed, prep["outNames"], prep["sizes"]]
]
...
TEinRoll[pattern_String, x_TTerm, shift_Integer, rest___] := Catch @ Block[{parsed, split, prep, axes, axisName, axisIdx, len, shft, lifted, lo, hi, padded, loPad, hiPad, term, finalParsed},
    parsed = parseEinxPattern[pattern];
    split = einxSplitArgs[{rest}];
    prep = einxPrepare[parsed, {x}, split["hints"], "elem"];
    If[ Length[parsed["bracketed"]] =!= 1,
        einxParseFail["TEinRoll needs exactly one bracketed axis", "Bracketed" -> parsed["bracketed"]]];
    ...
    term = If[ shft === 0,
        ...
        Block[{shrinkRanges, padRanges, rank = Length[axes]},
            shrinkRanges[lo_, hi_] := Table[If[k === axisIdx, {lo, hi}, {0, prep["sizes"][axes[[k]]]}], {k, rank}];
            padRanges[loPad_, hiPad_] := Table[If[k === axisIdx, {loPad, hiPad}, {0, 0}], {k, rank}];
...
einxReductionVerb[verbName_String, kind_String, neutral_, pattern_String, x_TTerm, rest_List] := Catch @ Block[{parsed, split, prep, term, reduced, axesAfter},
    parsed = parseEinxPattern[pattern];
    split = einxSplitArgs[rest];
    prep = einxPrepare[parsed, {x}, split["hints"], "reduce"];
    term = First @ prep["lifted"];
    {reduced, axesAfter} = einxReducePass[term, prep["globalAxes"], prep["reduceNames"], kind];
    einxFinalize[reduced, axesAfter, parsed, prep["outNames"], prep["sizes"]]
]

TEinSum[pattern_String, x_TTerm, rest___] := einxReductionVerb["TEinSum", "SUM", 0.0, pattern, x, {rest}]

TEinMax[pattern_String, x_TTerm, rest___] := einxReductionVerb["TEinMax", "MAX", -Infinity, pattern, x, {rest}]

(* TEinMin = -max(-x) *)
TEinMin[pattern_String, x_TTerm, rest___] := -einxReductionVerb["TEinMin", "MAX", -Infinity, pattern, -x, {rest}]

(* Helper: count of elements reduced by `reduceNames` (product of
   their sizes). *)
einxReduceCount[prep_] := Times @@ (prep["sizes"] /@ prep["reduceNames"])

TEinMean[pattern_String, x_TTerm, rest___] := Catch @ Block[{parsed, split, prep, term, reduced, axesAfter, count},
    parsed = parseEinxPattern[pattern];
    split = einxSplitArgs[{rest}];
    prep = einxPrepare[parsed, {x}, split["hints"], "reduce"];
    term = First @ prep["lifted"];
    {reduced, axesAfter} = einxReducePass[term, prep["globalAxes"], prep["reduceNames"], "SUM"];
...
(* TEinProd = exp(sum(log(x))).  Requires x > 0; this is the einx
   contract too. *)
TEinProd[pattern_String, x_TTerm, rest___] := Catch @ Block[{parsed, split, prep, term, reduced, axesAfter},
    parsed = parseEinxPattern[pattern];
    split = einxSplitArgs[{rest}];
    prep = einxPrepare[parsed, {x}, split["hints"], "reduce"];
...
TEinAny[pattern_String, x_TTerm, rest___] := einxReductionVerb["TEinAny", "MAX", 0.0, pattern, x, {rest}]

TEinAll[pattern_String, x_TTerm, rest___] := 1 - einxReductionVerb["TEinAll", "MAX", 0.0, pattern, 1 - x, {rest}]

(* Var = E[(x - E[x])^2] over the bracketed axes. *)
TEinVar[pattern_String, x_TTerm, rest___] := Catch @ Block[{parsed, split, prep, term, meanReduced, axesAfter, count, meanBroadcast, diff, sqReduced, axesAfter2},
    parsed = parseEinxPattern[pattern];
    split = einxSplitArgs[{rest}];
    prep = einxPrepare[parsed, {x}, split["hints"], "reduce"];
    term = First @ prep["lifted"];
    count = einxReduceCount[prep];
    {meanReduced, axesAfter} = einxReducePass[term, prep["globalAxes"], prep["reduceNames"], "SUM"];
    meanBroadcast = einxBroadcastBack[meanReduced / count, axesAfter, prep["globalAxes"], prep["sizes"]];
    diff = term - meanBroadcast;
    {sqReduced, axesAfter2} = einxReducePass[diff * diff, prep["globalAxes"], prep["reduceNames"], "SUM"];
...
einxBroadcastBack[term_, curAxes_, globalAxes_, sizes_] := Block[{insertShape, expanded},
    If[ curAxes === globalAxes,
        Return[term, Block]];
    insertShape = Map[If[MemberQ[curAxes, #], sizes[#], 1] &, globalAxes];
    expanded = TUOpReshape[
        ...
        Block[{permOrder = SortBy[Range[Length[curAxes]], Position[globalAxes, curAxes[[#]]][[1, 1]] &]},
            If[permOrder === Range[Length[curAxes]], term, TUOpPermute[term, permOrder - 1]]
        ],
        insertShape
    ];
...
TEinLogSumExp[pattern_String, x_TTerm, rest___] := Catch @ Block[{parsed, split, prep, term, maxR, axesAfterM, maxB, shifted, eShifted, sumE, axesAfterS, sumLog},
    parsed = parseEinxPattern[pattern];
    split = einxSplitArgs[{rest}];
    prep = einxPrepare[parsed, {x}, split["hints"], "reduce"];
    term = First @ prep["lifted"];
    {maxR, axesAfterM} = einxReducePass[term, prep["globalAxes"], prep["reduceNames"], "MAX"];
    maxB = einxBroadcastBack[maxR, axesAfterM, prep["globalAxes"], prep["sizes"]];
    shifted = term - maxB;
    eShifted = Exp[shifted];
    {sumE, axesAfterS} = einxReducePass[eShifted, prep["globalAxes"], prep["reduceNames"], "SUM"];
...
TEinSoftmax[pattern_String, x_TTerm, rest___] := Catch @ Block[{parsed, finalParsed, split, prep, term, maxR, axesM, maxB, shifted, eShifted, sumE, axesS, sumB, result},
    parsed = parseEinxPattern[pattern];
    finalParsed = If[parsed["outSpecified"], parsed, <|parsed, "out" -> First[parsed["in"]], "outSpecified" -> True|>];
    split = einxSplitArgs[{rest}];
    prep = einxPrepare[finalParsed, {x}, split["hints"], "named-reduce"];
    term = First @ prep["lifted"];
    {maxR, axesM} = einxReducePass[term, prep["globalAxes"], prep["reduceNames"], "MAX"];
    maxB = einxBroadcastBack[maxR, axesM, prep["globalAxes"], prep["sizes"]];
    shifted = term - maxB;
    eShifted = Exp[shifted];
    {sumE, axesS} = einxReducePass[eShifted, prep["globalAxes"], prep["reduceNames"], "SUM"];
    sumB = einxBroadcastBack[sumE, axesS, prep["globalAxes"], prep["sizes"]];
    result = eShifted / sumB;
    einxFinalize[result, prep["globalAxes"], finalParsed, prep["outNames"], prep["sizes"]]
]

TEinLayerNorm[pattern_String, x_TTerm, rest___] := TEinLayerNorm[pattern, x, 1.0*^-5, rest]
TEinLayerNorm[pattern_String, x_TTerm, eps_ ? NumericQ, rest___] := Catch @ Block[{parsed, finalParsed, split, prep, term, count, meanR, axesM, meanB, diff, varR, axesV, varB, normalized},
    parsed = parseEinxPattern[pattern];
    finalParsed = If[parsed["outSpecified"], parsed, <|parsed, "out" -> First[parsed["in"]], "outSpecified" -> True|>];
    split = einxSplitArgs[{rest}];
    prep = einxPrepare[finalParsed, {x}, split["hints"], "named-reduce"];
    term = First @ prep["lifted"];
    count = einxReduceCount[prep];
    {meanR, axesM} = einxReducePass[term, prep["globalAxes"], prep["reduceNames"], "SUM"];
    meanB = einxBroadcastBack[meanR / count, axesM, prep["globalAxes"], prep["sizes"]];
    diff = term - meanB;
    {varR, axesV} = einxReducePass[diff * diff, prep["globalAxes"], prep["reduceNames"], "SUM"];
    varB = einxBroadcastBack[varR / count, axesV, prep["globalAxes"], prep["sizes"]];
    normalized = diff / Sqrt[varB + eps];
    einxFinalize[normalized, prep["globalAxes"], finalParsed, prep["outNames"], prep["sizes"]]
]

TEinRMSNorm[pattern_String, x_TTerm, rest___] := TEinRMSNorm[pattern, x, 1.0*^-5, rest]
TEinRMSNorm[pattern_String, x_TTerm, eps_ ? NumericQ, rest___] := Catch @ Block[{parsed, finalParsed, split, prep, term, count, msR, axesV, msB, result},
    parsed = parseEinxPattern[pattern];
    finalParsed = If[parsed["outSpecified"], parsed, <|parsed, "out" -> First[parsed["in"]], "outSpecified" -> True|>];
    split = einxSplitArgs[{rest}];
    prep = einxPrepare[finalParsed, {x}, split["hints"], "named-reduce"];
    term = First @ prep["lifted"];
    count = einxReduceCount[prep];
    {msR, axesV} = einxReducePass[term * term, prep["globalAxes"], prep["reduceNames"], "SUM"];
...
einxElementVerb[combiner_, parsed_, args_List, hints_Association] := Block[{prep, combined},
...
(* TEinWhere -- cond * a + (1 - cond) * b.  Inputs are {cond, a, b}
   in the order given. *)
TEinWhere[pattern_String, rest__] := Catch @ Block[{parsed, split, prep, cond, a, b, lifted, result},
    parsed = parseEinxPattern[pattern];
    split = einxSplitArgs[{rest}];
    If[ Length[split["tensors"]] =!= 3,
        einxParseFail["TEinWhere needs three tensors (cond, a, b)", "Got" -> Length[split["tensors"]]]];
    prep = einxPrepare[parsed, split["tensors"], split["hints"], "elem"];
    lifted = prep["lifted"];
    {cond, a, b} = lifted;
    result = cond * a + (1 - cond) * b;
    einxFinalize[result, prep["globalAxes"], parsed, prep["outNames"], prep["sizes"]]
]

(* === contraction (Dot) ========================================== *)

TEinDot[pattern_String, rest__] := Catch @ Block[{parsed, split, prep, lifted, product, reduced, axesAfter},
    parsed = parseEinxPattern[pattern];
    If[ ! parsed["outSpecified"],
    ...
    {reduced, axesAfter} = einxReducePass[product, prep["globalAxes"], prep["reduceNames"], "SUM"];
    einxFinalize[reduced, axesAfter, parsed, prep["outNames"], prep["sizes"]]
]
...
einxIndexingNotImplemented[pattern_, reason_] := Failure["EinxNotImplemented", <|"Reason" -> reason, "Pattern" -> pattern, "See" -> "TEmbedding note in NN.wl: dynamic-idx gather needs UOP_GATHER"|>]
...
TEinGetAt[pattern_String, table_TTerm, idx_List, rest___] := Catch @ Block[{parsed, split, tableShape, idxLen, d, rows, padded},
    If[ ! VectorQ[idx, IntegerQ],
        Return[einxIndexingNotImplemented[pattern, "idx must be a host-side List[Integer]"], Block]];
    parsed = parseEinxPattern[pattern];
    split = einxSplitArgs[{rest}];
    ...
    Which[
        Length[tableShape] === 2,
            d = tableShape[[2]];
            idxLen = Length[idx];
            rows = TUOpReshape[TUOpShrink[table, {{#, # + 1}, {0, d}}], {1, d}] & /@ idx;
            padded = MapIndexed[TUOpPad[#1, {{First[#2] - 1, idxLen - First[#2]}, {0, 0}}] &, rows];
            Total[padded],
        Length[tableShape] === 1,
            idxLen = Length[idx];
            rows = TUOpReshape[TUOpShrink[table, {{#, # + 1}}], {1}] & /@ idx;
            padded = MapIndexed[TUOpPad[#1, {{First[#2] - 1, idxLen - First[#2]}}] &, rows];
            Total[padded],
        True,
            einxIndexingNotImplemented[pattern, "rank " <> ToString[Length[tableShape]] <> " gather not yet supported"]
    ]
]
TEinGetAt[pattern_String, _TTerm, _TTerm, ___] := einxIndexingNotImplemented[pattern, "dynamic-tensor idx (use a host List[Integer])"]
...
*)
einxScatterCore[op_Symbol, pattern_String, x_TTerm, idx_List, vals_TTerm] := Catch @ Block[{xShape, valsShape, dimsTail, len, scatter, mask, base},
    If[ ! VectorQ[idx, IntegerQ],
        Return[einxIndexingNotImplemented[pattern, "idx must be a host-side List[Integer]"], Block]];
    xShape = tUopShape[x];
    ...
    len = Length[idx];
    If[ valsShape[[1]] =!= len,
        einxParseFail["scatter: vals leading dim must equal Length[idx]", "ValsShape" -> valsShape, "IdxLen" -> len]];
    dimsTail = Rest[xShape];
    If[ Rest[valsShape] =!= dimsTail,
        einxParseFail["scatter: vals tail shape must match table tail", "ValsShape" -> valsShape, "XShape" -> xShape]];
    ...
    scatter = Total @ MapIndexed[
        Function[{i, pos},
            Block[{row, rowShape, padRanges},
                ...
                row = TUOpReshape[TUOpShrink[vals, Prepend[Map[{0, #} &, dimsTail], {First[pos] - 1, First[pos]}]], rowShape];
                padRanges = Prepend[Map[{0, 0} &, dimsTail], {i, xShape[[1]] - i - 1}];
...
einxConstLike[v_, shape_List] := TUOpExpand[TUOpReshape[TUOpConst[v], ConstantArray[1, Length[shape]]], shape]

TEinSetAt[pattern_String, x_TTerm, idx_List, vals_TTerm, rest___] := einxScatterCore[Set, pattern, x, idx, vals]
TEinSetAt[pattern_String, _TTerm, _TTerm, _TTerm, ___] := einxIndexingNotImplemented[pattern, "dynamic-tensor idx (use a host List[Integer])"]

TEinAddAt[pattern_String, x_TTerm, idx_List, vals_TTerm, rest___] := einxScatterCore[Plus, pattern, x, idx, vals]
TEinAddAt[pattern_String, _TTerm, _TTerm, _TTerm, ___] := einxIndexingNotImplemented[pattern, "dynamic-tensor idx (use a host List[Integer])"]

TEinSubAt[pattern_String, x_TTerm, idx_List, vals_TTerm, rest___] := einxScatterCore[Subtract, pattern, x, idx, vals]
TEinSubAt[pattern_String, _TTerm, _TTerm, _TTerm, ___] := einxIndexingNotImplemented[pattern, "dynamic-tensor idx (use a host List[Integer])"]
