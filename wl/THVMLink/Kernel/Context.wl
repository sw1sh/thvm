...
SetUsage[TContext, "TContext[id$] is an opaque handle to a runtime context (heap, book, defs, alo state, tensors, kernels, backends).
Slot 0 is the default singleton; slots 1..15 are user-allocated via TContextNew[]."];
SetUsage[$TContext, "$TContext is the default context for the context-polymorphic API; initial value TContext[0].
Rebind via Block[{$TContext = ctx$}, $$] or TInContext[ctx$, $$]."];
SetUsage[TContextNew, "TContextNew[] allocates a fresh context with the default device and returns TContext[id$].
...
SetUsage[TContextDestroy, "TContextDestroy[TContext[id$]] frees the C-side state for that context and returns the destroyed slot id.
Slot 0 (default) is preserved and returns 0 as a no-op; use TFree[] to tear it down."];
SetUsage[TContextCurrent, "TContextCurrent[] returns the C-side current context as TContext[id$]."];
SetUsage[TContextList, "TContextList[] returns all allocated contexts as a list of TContext[id$] (slot 0 first)."];
SetUsage[TInContext, "TInContext[ctx$, body$] evaluates body$ with $TContext rebound to ctx$ and the C-side current context switched to ctx$ for the duration.
...
TContextNew[device_] := With[{slot = $contextCreateFn[deviceCode[device]]},
    If[ slot === 0,
        Failure["TContextOverflow", <|"Message" -> "TContextNew: context table full (CONTEXTS_CAP=16)"|>],
...
TContextList[] := Module[{prev = $contextCurrentFn[], live},
    live = Table[
        ...
        If[$contextCurrentFn[] === slot, TContext[slot], Nothing],
        {slot, 1, 15}
    ];
...
withCtx[TContext[slot_Integer], expr_] := Module[{prev},
    ...
    Internal`WithLocalSettings[Null, expr, $contextSelectFn[prev]]
]
...
withTermCtx[t_, expr_] := With[{c = ttermCtx[t]},
    If[c === $contextCurrentFn[], expr, withCtx[TContext[c], expr]]
]
