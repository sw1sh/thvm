...
SetUsage[TAOTEmit, "TAOTEmit[name$] returns the C source string that thvm_aot_emit_program produces for the def registered under name$.
Inspect, save, or compile manually; TAOTCompile and TAOTRun automate those steps on top of this lower-level surface."];

SetUsage[TAOTCompile, "TAOTCompile[name$] emits the def under name$, wraps it with the thvm runtime plus an aot_program_<name>_run entry, invokes clang to produce a dylib, stashes the path so TAOTRun can dlopen it, and returns the dylib path string.
Caching is by content, so an unchanged source reuses the same path and skips the clang call."];

SetUsage[TAOTRun, "TAOTRun[name$, input$] dlopens the AOT'd dylib for name$ (TAOTCompile'd first), invokes its run entry with input$ as the argument Term, and returns the result wrapped as a TTerm. input$ may be a TTerm or a raw Integer (wrapped as TNum).
...
SetUsage[TAOTPath, "TAOTPath[name$] returns the dylib path stashed by TAOTCompile[name$], or Missing[\"NotCompiled\"] if the def has never been TAOTCompile'd."];

SetUsage[TAOTSpSolve, "TAOTSpSolve[cnf$, nVars$] runs the Survey Propagation plus decimation SAT solver via Metal and returns {status$, assignment$}.
...
SetUsage[TAOTSurveyPropagate, "TAOTSurveyPropagate[cnf$, nVars$] runs Survey Propagation on the CNF formula via Metal and returns the per-edge final eta vector (length equals the total literals across clauses) after convergence or the iteration cap.
...
SetUsage[TAOTSatBitmask, "TAOTSatBitmask[cnf$, nVars$] evaluates the CNF formula at every assignment in [0, 2^nVars$) on Metal via the aot_cnf_bitmask kernel.
...
SetUsage[TAOTIcCollapse, "TAOTIcCollapse[term$, depth$] dispatches the static aot_ic_collapse PSO with grid 2^depth$ over a BOOK_HEAP-rooted SUP-tree at term$ (for example the result of TAOTRun[$$, Method \[Rule] \"Metal\"] run with THVM_AOT_METAL_KEEP_BOOK=1 set).
...
SetUsage[TAOTBatchOp2Fold, "TAOTBatchOp2Fold[rootLocs$] dispatches the batch kernel aot_eval_op2_fold_batch over a list of book_heap locations, each pointing at an OP2(NUM, NUM) cell, and returns the corresponding list of folded NUM TTerms.
...
$aotEmitFn := $aotEmitFn = load["thvm_wl_aot_emit_program", {Integer, "UTF8String"}, "UTF8String"]
$aotCompileFn := $aotCompileFn = load["thvm_wl_aot_compile", {Integer, "UTF8String"}, "UTF8String"]
$aotRunFn := $aotRunFn = load["thvm_wl_aot_run", {"UTF8String", "UTF8String", Integer}, Integer]
$aotRun4Fn := $aotRun4Fn = load["thvm_wl_aot_run4", {"UTF8String", "UTF8String", Integer, Integer, Integer, Integer}, Integer]
$aotRun4PooledFn := $aotRun4PooledFn = load["thvm_wl_aot_run4_pooled", {"UTF8String", "UTF8String", Integer, Integer, Integer, Integer, Integer}, Integer]
...
$aotMetalRun4Fn := $aotMetalRun4Fn = load["thvm_wl_aot_metal_run4", {Integer, "UTF8String", Integer, Integer, Integer, Integer}, Integer];
...
$aotMetalRunNFn := $aotMetalRunNFn = load["thvm_wl_aot_metal_run_n", {Integer, "UTF8String", {Integer, 1}}, Integer];

$aotMetalBatchOp2Fn := $aotMetalBatchOp2Fn = load["thvm_wl_aot_metal_op2_fold_batch", {{Integer, 1}}, {Integer, 1}];
...
(* Parallel cnf+collapse on a BOOK_HEAP-rooted SUP-tree, dispatched
   ...
   filter. *)
$aotIcCollapseFn := $aotIcCollapseFn = load["thvm_wl_aot_ic_collapse", {Integer, Integer}, {Integer, 1}];
...
$aotCnfBitmaskFn := $aotCnfBitmaskFn = load["thvm_wl_aot_cnf_bitmask", {{Integer, 1}, {Integer, 1}, Integer}, {Integer, 1}];

cnfToBitmasks[cnf_List, nVars_Integer] := Module[{posMasks, negMasks},
    posMasks = Table[BitOr @@ Append[Cases[clause, lit_ /; lit > 0 :> 2^(lit - 1)], 0], {clause, cnf}];
    negMasks = Table[BitOr @@ Append[Cases[clause, lit_ /; lit < 0 :> 2^(-lit - 1)], 0], {clause, cnf}];
    {posMasks, negMasks}
]
...
$aotSpRunFn := $aotSpRunFn = load["thvm_wl_aot_sp_run", {{Integer, 1}, {Integer, 1}, Integer, Integer, Real, Real}, {Real, 1}];

Options[TAOTSurveyPropagate] = {"MaxIters" -> 100, "Damping" -> 0.5, "Threshold" -> 0.001}
$aotSpSolveFn := $aotSpSolveFn = load["thvm_wl_aot_sp_solve", {{Integer, 1}, {Integer, 1}, Integer, Integer, Real, Real}, {Integer, 1}];

Options[TAOTSpSolve] = {"MaxIters" -> 100, "Damping" -> 0.5, "Threshold" -> 0.001}
TAOTSpSolve[cnf_List, nVars_Integer, opts : OptionsPattern[]] := Module[{flat, bounds, maxIters, damping, threshold, raw, status, assignment, statusStr},
    ensureInit[];
    flat = Flatten[cnf];
    ...
    raw = $aotSpSolveFn[Developer`ToPackedArray[flat, Integer], Developer`ToPackedArray[bounds, Integer], nVars, maxIters, N[damping], N[threshold]];
    status = First[raw];
    assignment = Rest[raw];
...
TAOTSurveyPropagate[cnf_List, nVars_Integer, opts : OptionsPattern[]] := Module[{flat, bounds, maxIters, damping, threshold},
    ensureInit[];
    flat = Flatten[cnf];
    bounds = Prepend[Accumulate[Length /@ cnf], 0];
    maxIters = OptionValue["MaxIters"];
    damping = OptionValue["Damping"];
    threshold = OptionValue["Threshold"];
    $aotSpRunFn[Developer`ToPackedArray[flat, Integer], Developer`ToPackedArray[bounds, Integer], nVars, maxIters, N[damping], N[threshold]]
]

TAOTSatBitmask[cnf_List, nVars_Integer] := Module[{pos, neg},
    ensureInit[];
    {pos, neg} = cnfToBitmasks[cnf, nVars];
    $aotCnfBitmaskFn[Developer`ToPackedArray[pos, Integer], Developer`ToPackedArray[neg, Integer], nVars]
]
...
TAOTBatchOp2Fold[rootLocs_List] := Module[{raws},
    ensureInit[];
    raws = $aotMetalBatchOp2Fn[Developer`ToPackedArray[rootLocs, Integer]];
    TTerm /@ raws
]
...
$aotMetalIcDefRunFn := $aotMetalIcDefRunFn = load["thvm_wl_aot_metal_ic_def_run", {Integer, {Integer, 1}}, Integer];

aotMetalIcDefRunImpl[name_String, args_List] := Module[{raws},
    raws = toRawArg /@ args;
    TTerm[$aotMetalIcDefRunFn[TDefName[name], Developer`ToPackedArray[raws, Integer]]]
]
...
methodHead[spec_] := Replace[spec, {_String -> spec, {h_String, ___} :> h, _ -> None}]
methodOpts[spec_] := Replace[spec, {{_String, o___} :> {o}, _ -> {}}]
...
aotCpuRunImpl[name_String, args_, spec_] := Module[{path, raws, slots, threads},
    path = TAOTPath[name];
    If[MissingQ[path], path = TAOTCompile[name]];
    If[ path === $Failed || MissingQ[path],
        Message[TAOTRun::method, spec]; Return[$Failed]];
    threads = Replace[spec, {_String -> 1, {_String, OrderlessPatternSequence["NumThreads" -> n_Integer, ___]} :> n, _ -> 1}];
    If[ threads <= 1,
        TAOTRun[name, args],
        raws = toRawArg /@ args;
        slots = PadRight[raws, 4, 0];
        TTerm[$aotRun4PooledFn[path, name, threads, slots[[1]], slots[[2]], slots[[3]], slots[[4]]]]
    ]
]
