...
TAtpLoadGnnScorer::usage = "TAtpLoadGnnScorer[path] loads a GCN scorer .safetensors file (saved by TAtpSaveGnnScorer) and returns the \"Model\" Association (W1 / Ws / Bh / Wout / Bout + Rounds / Hidden / NMax).  TAtpSetGnnScorer[path] = TAtpSetGnnScorer[TAtpLoadGnnScorer[path]].";
...
(* TATP-style axioms-first order: TFindProof[{ax1, ...}, conjecture]
   mirrors TATP[{lhs == rhs, ...}, conjecture].  Fires only when the
   first argument is a List and the second is a single equation-shaped
   expression -- never a return spec (String / list of Strings / All),
   so the single-arg completion form TFindProof[{axioms}, "Lemmas"]
   is untouched.  A list of conjectures against a single bare axiom
   stays expressible as TFindProof[{c1, c2}, {axiom}]. *)
TFindProof[axioms_List, conjecture : (_Equal | _Unequal | _ForAll | _Rule
        | _TwoWayRule
        | Inactive[Equal][_, _] | Inactive[Unequal][_, _]
        | Inactive[Rule][_, _] | Inactive[TwoWayRule][_, _]),
        opts:OptionsPattern[]] :=
...
TFindProof[axioms_List, conjecture : (_Equal | _Unequal | _ForAll | _Rule
        ...
        | Inactive[Rule][_, _] | Inactive[TwoWayRule][_, _]),
        returnSpec_?atpReturnSpecQ, opts:OptionsPattern[]] :=
    TFindProof[conjecture, axioms, returnSpec, opts];
