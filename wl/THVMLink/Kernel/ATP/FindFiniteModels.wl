...
SetUsage[TFindFiniteModels, "TFindFiniteModels[rels$] finds multiplication-table models consistent with the relations rels$ over each operator, assuming each variable takes one of two values.
...
mixedTupleFromIndex[index_, bs_] := Rest @ FoldPairList[{Quotient[#1, #2], Mod[#1, #2]}&, index, Reverse @ (FoldList[Times, 1, #]&) @ Reverse @ bs]
...
MapTake[f_, expr_List, n_Integer] := With[{first = First @ expr, rest = Rest @ expr, m = Max[n, 0]},
...
ParallelMapTake[f_, expr_, Infinity] := Catenate @ ParallelMap[f, expr, DistributedContexts -> Automatic]
ParallelMapTake[f_, expr_List, n_] := Module[{firstBatch, restBatch},
    {firstBatch, restBatch} = TakeDrop[expr, UpTo[$ProcessorCount]];
    Take[
        Catenate @ ParallelMap[MapTake[f, Prepend[restBatch, #], n]&, firstBatch, DistributedContexts -> Automatic],
        n
    ]
]

$compilationTarget := $compilationTarget = If[noCCompilerQ[], "WVM", "C"]
noCCompilerQ[] := TrueQ @ Quiet @ Check[Compile[{}, 1 + 1, CompilationTarget -> "C"], True, CCompilerDriver`CreateLibrary::nocomp]

Options[TFindFiniteModels] = {"Parallelize" -> False, Method -> Automatic, MaxItems -> Infinity, "ReverseOrdering" -> False}

TFindFiniteModels[relations_, k_Integer : 2, prop_String : "Association", opts : OptionsPattern[]] := Catch[
...
findFiniteModels[relations_, arities_Association, vars_List, k_Integer, prop_, OptionsPattern[]] := Module[{relationFunction, indices},
...
satIndices[relationFunction_, arities_Association, vars_List, k_] := Module[{ops, cellSpec, allCells, vals, satEval, satEqual, satUnequal, totality, closure, formula, varList, instances},
    ops = Keys[arities];
...
ccEncode[assign_, cellSpec_, k_] := Map[spec |-> FromDigits[Map[tup |-> assign[Key[{spec[[1]], tup}]], spec[[3]]], k], cellSpec]
...
ccCDecide := ccCDecide = load["thvm_wl_cc_decide", {{Integer, 1}, {Integer, 1}, {Integer, 1}}, Integer]
...
ccEncodeTerms[terms_] := Module[{syms, heads, ids = <||>, count = 0, recs = Internal`Bag[], emit},
...
ccCDecideQ[eqs_, diseqs_] := Module[{pre, eqsC, diseqsC, allTerms, stream, idOf, eqFlat, neFlat},
...
ccPreprocess[eqs_, diseqs_] := If[ MemberQ[eqs, False] || MemberQ[diseqs, False],
    "UNSAT",
    {DeleteCases[eqs, True], DeleteCases[diseqs, True]}
]

ccIndices[decideQ_, relationFunction_, arities_Association, vars_List, k_, maxItems_] := Module[{ops, cellSpec, allCells, nCells, distinct, groundEqs, assigned, fullClosure, results, dfs},
    ops = Keys[arities];
    cellSpec = Map[op |-> {op, arities[op], Tuples[Range[0, k - 1], arities[op]]}, ops];
    allCells = Catenate @ Map[spec |-> Map[tup |-> {spec[[1]], tup}, spec[[3]]], cellSpec];
...
ccCIndicesIncremental[relationFunction_, arities_Association, vars_List, k_, maxItems_] := Module[{ops, cellSpec, allCells, nCells, groundEqs, heads, handle, termNode, intern, assertAtom, assigned, fullClosure, results, dfs, out},
    ops = Keys[arities];
    cellSpec = Map[op |-> {op, arities[op], Tuples[Range[0, k - 1], arities[op]]}, ops];
    allCells = Catenate @ Map[spec |-> Map[tup |-> {spec[[1]], tup}, spec[[3]]], cellSpec];
    nCells = Length[allCells];
...
ffmepSolve := ffmepSolve = load["thvm_wl_ffmep_solve", {{Integer, 1}, {Integer, 1}, {Integer, 1}, {Integer, 1}, {Integer, 1}, Integer, Integer}, {Integer, 2}]
...
epClauseDB[relationFunction_, arities_Association, vars_List, k_Integer] := Module[{expr, digits, toDigitLogic, ops, sizes, offsets},
    digits /: Plus[digits[a_], b_digits] := digits[Map[# + b&, a]];
    digits /: Equal[digits[a_], b_digits] := digits[Map[# - b&, a]] == 0;
    digits /: digits[a_] + b_ := digits[a + b];
    digits /: digits[a_] * b_ := digits[a * b];
...
epcIndices[relationFunction_, arities_Association, vars_List, k_, maxItems_] := Module[{db, expr, ops, canonOps, canonSizes, offsetOf, encodeLit, encodeClause, clauseLists, litFlat, clauseOff, setOff, setClause, raw, canonToOrig},
...
epcFlatten[clauseLists_] := Module[{flatClauses, clauseLens, litFlat, clauseOff, setLens, setOff, setClause},
...
(* ----- "Z3" Method -----
   ...
   produce. *)
z3Indices[relationFunction_, arities_Association, vars_List, k_, maxItems_] := Module[{ops, z3name, handle, cellTuples, z3cell, z3term, ranges, ground, solver, step, results},
