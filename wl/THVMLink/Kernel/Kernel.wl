...
TKernelAutotune::usage = "TKernelAutotune[kid] benchmarks every TKernelProposed candidate against the no-opt baseline (THVM_AUTOTUNE_RUNS dispatches each, default 5, min wallclock), applies the winning TOpt to the kernel's C-side KernelAxes, and returns the resulting TKernelOpts.  Metal LOCAL winners record both LOCAL and the matching GLOBAL mark; Metal TC winners record the chosen MMA tile size.  Because axes live on a shared schedule slot, the winner auto-applies to other kernels with the same structural key.  Returns the unchanged TKernelOpts (no opts applied) if no candidate beat baseline.";
