...
GeneralUtilities`SetUsage[TDot, "TDot[a$, b$] = TSum[TUOpMul[a$, b$]]."];
GeneralUtilities`SetUsage[TMatVec, "TMatVec[W$, x$] computes W$ @ x$ where W$ is {out, in} and x$ is {in} (rank-1) or {1, in} (rank-2), giving {out}. Expand-broadcast then sum-reduce along the inner axis."];
GeneralUtilities`SetUsage[TMatMul, "TMatMul[A$, B$] computes A$ @ B$ where A$ is {M, K} and B$ is {K, N}, giving {M, N}. Lowered as reshape + expand to a common {M, K, N} shape, elementwise multiply, then sum-reduce along axis 1; this pattern is recognised and routed to cblas_sgemm."];
GeneralUtilities`SetUsage[TLinear, "TLinear[x$, W$, b$] computes x$ @ W$ + b$ where x$ is {$$, M, K}, W$ is {K, N}, and b$ is {N} (or None for bias-free). The bias is reshaped to {1, $$, 1, N} and expanded to the matmul output shape. Standard nn.Linear analogue (tinygrad's Tensor.linear)."];
...
GeneralUtilities`SetUsage[TLog, "TLog[x$] is the elementwise natural log, lowered as log2(x$) * ln(2) since the runtime has log2 but no native log. Kept as a public alias; new code should write Log[x$]."];
