...
| `"LeafTiebreakFacegate"` | Leaf-tie-break face gate: skip the `"LeafTiebreak"` oriented-first flip when the oriented partner is overlapped on its Waldmeister-distinguished face but the permutation partner on its reverse face -- there `thvm`'s DFS arrival already matches Waldmeister's formation order. Auto-on under `"FormationFifo"`; `Automatic` leaves it at FormationFifo's value. | (Meredith 1047) |
| `"CommDropDupClassGate"` | Inner-swap anchor gate for the `"CommDropDup"` re-age: skip the slot-15-term re-age when its smallest-keyed successor is a Meredith-harmful anchor Waldmeister emits *after* the slot-15-term (the permutation class `(x.y).y = (y.x).y` or the slot-15-rotate `x.(y.x) = (x.y).x`). Neither shape is a soa anchor, so soa stays byte-identical. Auto-on under `"FormationFifo"`. | (Meredith 4190) |
| `"CorankOwnArr"` | Two-face co-rank correction: re-key a Waldmeister-reverse-face overlap of the `(x.(x.x)).y = y.y` partner onto its OWN tops-DFS arrival when it is a distinct (non-double-MGU) surviving CP, matching Waldmeister's independent aging. Auto-on under `"FormationFifo"`. | (Meredith 6078) |
| `"MeredDmgu"`    | Shared-reverse-face double-MGU defer: in a weight-120 tops-A equation-tree band, defer the chain-head (newest-equation) combo=0 CP that shares a reverse-face leaf with an older equation's combo=0 CP -- Waldmeister ages that content as the older equation's late second MGU, not at the band head. Auto-on under `"FormationFifo"`. | (Meredith band) |
| `"FormationFifo"`| The SINGLE knob enabling the faithful Waldmeister CP-formation order. ON it turns on the full 13-flag k3-arrival faithful stack: the four scoped emission-order re-key passes above (`"LeafTiebreak"` / `"RevfaceGroup"` / `"PosGroup"` / `"CubeArrival"`) plus `"LeafTiebreakFacegate"`, `"CommDropDupClassGate"`, `"CorankOwnArr"`, `"MeredDmgu"`, the distinguished-direction E-subsumption (`"EsetDistdir"`, below), and the within-leaf drain/cube-order corrections (band-interleave, drain-chainpos, drain-revface, revface-cubeorder). Together they reproduce Waldmeister's combined-superposition-scan emission order: Waldmeister stamps each surviving critical pair `w2 = ++CPNr` at insertion (NewClassification.c `C_Classify`), per overlap position every rule-tree partner (discrimination-tree leaf-arrival order) precedes every equation-tree partner (Unifikation1.c `U1_KPsBildenZuRegel`), so a multiply-formed term's surviving copy inherits Waldmeister's `CPNr` age. The CPNr lifecycle around it -- re-classification preserving the age (`C_ReClassify`) and re-derivation re-stamping it (`KPV_IROpferBehandeln`) -- is already ported. Atop the base `"CPSide"` + `"FlatSubsume"` + `"CommReage"` + `"CommDropDup"` knobs it reaches the FULL soa proof (firstdiv 2808 -- Waldmeister saturates at pick 2807) and MeredithAxioms `OrAssociativity` firstdiv 6110. The five auto-on knobs above each take an `Automatic`/`True`/`False` tri-state: `Automatic` (the default) leaves them at FormationFifo's value, `False` overrides one back off, `True` forces it on without the rest of the stack. | 2808 (full soa) |
...
Distinct from the emission-order family, `"EsetDistdir"` is a subsumption-faithfulness knob -- it changes which equations the E-set discards as subsumed, not how CPs are ranked. It ports Waldmeister's distinguished-direction E-set subsumption (`Interreduktion.c:261`): test each old equation only in its distinguished (stored) orientation, dropping the two subject-swapped match attempts of the general 4-way flat subsumer. This retires the earlier content-gate workaround and advances MeredithAxioms `OrAssociativity` first-divergence 6081 -> 6110. Auto-on under `"FormationFifo"`; `Automatic` (the default) leaves it at FormationFifo's value.
...
```wl
{19, 125, 288, 290, 966, 1320, 1505, 2808}
```
<!-- => the soa first-divergence as the knobs stack: bare 19, then +CPSide/FlatSubsume,
        +CommReage, +CommDropDup, +RevfaceGroup, +PosGroup, +CubeArrival, then the
        full FormationFifo stack reaching the complete soa proof (firstdiv 2808) -->

The full soa-validated set that reproduces Waldmeister's complete selection order is `"CPSide"` + `"FlatSubsume"` + `"CommReage"` + `"CommDropDup"` + the k3-arrival corrections (`"LeafTiebreak"` / `"RevfaceGroup"` / `"PosGroup"` / `"CubeArrival"` / `"LeafTiebreakFacegate"` / `"CommDropDupClassGate"` / `"CorankOwnArr"` / `"MeredDmgu"`) + `"EsetDistdir"`, all `-> True` -- equivalently `"FormationFifo" -> True`, the single-knob shorthand for the whole stack:
