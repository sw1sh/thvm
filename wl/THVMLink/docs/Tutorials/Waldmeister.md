...
| `"FormationFifo"`| The SINGLE knob enabling the faithful Waldmeister CP-formation order. ON it turns on the four scoped k3-arrival re-key passes above (`"LeafTiebreak"` / `"RevfaceGroup"` / `"PosGroup"` / `"CubeArrival"`) -- exactly equivalent to setting those four flags. Together they reproduce Waldmeister's combined-superposition-scan emission order: Waldmeister stamps each surviving critical pair `w2 = ++CPNr` at insertion (NewClassification.c `C_Classify`), per overlap position every rule-tree partner (discrimination-tree leaf-arrival order) precedes every equation-tree partner (Unifikation1.c `U1_KPsBildenZuRegel`), so a multiply-formed term's surviving copy inherits Waldmeister's `CPNr` age. The CPNr lifecycle around it -- re-classification preserving the age (`C_ReClassify`) and re-derivation re-stamping it (`KPV_IROpferBehandeln`) -- is already ported. Atop the base `"CPSide"` + `"FlatSubsume"` + `"CommReage"` + `"CommDropDup"` knobs it reaches selection 1504 byte-identical. | 1505 |
