...
The soa-validated faithful set that reproduces WM's selection order through 1504 selections is `"CPSide"` + `"FlatSubsume"` + `"CommReage"` + `"CommDropDup"` + `"LeafTiebreak"` + `"RevfaceGroup"` + `"PosGroup"` + `"CubeArrival"` all `-> True`.  `"CommSubsume"` (diagnostic, regresses) and `"CommDefer"` (superseded by `"CommReage"`) are excluded from it.
