...
The soa-validated faithful set that reproduces WM's selection order through 1319 selections is `"CPSide"` + `"FlatSubsume"` + `"CommReage"` + `"CommDropDup"` + `"LeafTiebreak"` + `"RevfaceGroup"` + `"PosGroup"` all `-> True`.  `"CommSubsume"` (diagnostic, regresses) and `"CommDefer"` (superseded by `"CommReage"`) are excluded from it.
