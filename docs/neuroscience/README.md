...
10. [11-skill-learning-survey.md](11-skill-learning-survey.md) -- the
   connectionist skill-learning survey: options framework, unsupervised
   skill discovery (DIAYN/DADS), manager-worker (FeUdal/HIRO/HAC),
   goal-conditioned + hindsight (HER), skill chaining + Deep Skill
   Graphs, object-centric frontier. Includes two follow-up sections on
   end-to-end variants (soft DAG over latent partition, refined to
   skill-coarsened MCTS) and attention-derived subgoals.
11. [12-modern-robotics-survey.md](12-modern-robotics-survey.md) --
   what modern robotics research (2022-present) has converged on:
   world models that learn and deploy (Dreamer V3, DayDreamer),
   diffusion / flow action policies, action chunking (ACT, RoboCat),
   latent action pretraining (LAPA, Genie), and online distillation.
12. [references.md](references.md) -- consolidated bibliography with
   links.

(Pages numbered 11 and 12 retain their filenames for citation
stability; only their index position renumbered after the earlier
intrinsic-goal-experiments synthesis page was moved out of this
directory.)
