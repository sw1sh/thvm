...
- 4b. Delete `KernelEntry.program[]` / KProgOp / `kernel_program_cache.c` / `uop_to_scalar.c`. The plan's earlier claim "Backends already consume `compute_root` post-F0 (`render_uop.c:1099`)" is empirically PARTIAL: `src/backend/cpu/jit.c:100-101` still hashes `ke->program` bytes as the JIT cache key and `jit.c:318` reads `ke->program[ke->n_ops-1].numel`, both firing when `cached_lift.store_root == 0` (lift declined: gemm/conv2d-only kernels, multi-output splices, oversized n_inputs). 219 reads of `ke->program` / `n_ops` / `ops_cap` survive in 17 files post-4b/2. Decomposition required (2026-05-14 reconnaissance):
    ...
    - **4b/3.a (decomposed further, 2026-05-14 recon)**: the ScalarUop arena (`KernelEntry.scalar_uops` / `n_scalar_uops` / `scalar_uops_cap` + 37-member `ScalarOp` enum) has FIVE direct readers across 293 references in 17 files. Cannot land in one commit:
        - **A1 BLOCKED**: instrumented `cpu_dispatch_kernel_inner` path 5 (scalar interpreter fallback) with a stderr trace; telemetry on the 8-suite run shows path 5 fires **~19 times**, not zero. Followup recon (commit `c92b2e21`, `THVM_TRACE_UOP_WALK_DECLINE=1`) shows all 19 collapse to one bucket: `lift_declined` (`kernel_lift_to_uop` returned `store_root == 0`). The walker itself never refuses a kernel the lifter accepts. Drilling into the lifter via `THVM_DUMP_LIFT_REJECT=1`: 12 of 19 rejects are `index/ndim-mismatch` on a 1D buffer (dim=[8]) with 2 ranges, in two shapes:
            - `range_extents=[4,8]` (10x) - broadcast leading, trailing range matches dim (offset = r[outer_rank-1]).
            - `range_extents=[2,4]` (2x) - flattened iter where prod(extents) == dim (offset = r[0]*4 + r[1]).
          Both need new fast-path handlers in `kernel_lift.c`'s `outer_rank > ndim` branch (currently only the leading-aligned case at line 367 lands). Closing these is the precondition for deleting `cpu_dispatch_scalar`. The remaining 7 declines come from non-instrumented lifter paths; next iteration needs broader lifter telemetry.
        - A2 (alternative): decouple `tile.c::tile_build_from_scalar` (~750 LOC) from the arena by rewriting it on `cached_lift.store_root` (the UOp DAG).
        - A3 (alternative): replace `kernel_program_cache::kp_hash`'s scalar_uops byte hash with a UOp-DAG-derived key. Already moot post-`4a7431b7` (kernel_program_cache deleted), so A3 is closed implicitly.
        - After A1+A2, the four remaining readers (kernel_lift main walker, propose.c, axis.c, capture.c) consume the arena indirectly; field deletion becomes tractable.
