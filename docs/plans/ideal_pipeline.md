...
| F6 | CPU-JIT via render_uop -> C; delete src/backend/cpu/op/*.c. | render_c.c, cpu/jit.c, cpu/op/* | -3000 | **render_c.c + cg_emit + Renderer DELETED** (eab0b9de -> 3b60fa7c, 16 commits, ~600 LOC removed in step 16 alone): cg_render_uop_kernel_c is the sole CPU-JIT emit path. Coverage: emit + clang compile-validate + dlopen+invoke + multi-input/unary/REDUCE_SUM/REDUCE_MAX/sqrt/BITCAST + full-suite env-on validation. WL FFI's diagnostic call retargeted. cpu/op/*.c (~1100 LOC, 21 files) deletion still gated on UOp-DAG CPU interpreter (cpu_interpret currently per-op when lifter declines). |
