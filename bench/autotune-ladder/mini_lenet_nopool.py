"""Level 15 -- no-pool mini-LeNet on tinygrad.

Mirrors bench/autotune-ladder/mini_lenet_nopool.wls.
Verifies pool's per-block cost on tinygrad's side by
comparison to tinygrad mini-LeNet (6 kernels).

Run:
    cd /Users/swish/src/tinygrad
    PYTHONPATH=. DEV=METAL python3 /Users/swish/src/thvm/bench/autotune-ladder/mini_lenet_nopool.py
"""
import os, sys, time
...
class MiniLeNetNoPool:
  def __init__(self):
    self.c1 = nn.Conv2d(1, 6, 5)
    # 28 -> 24, 6 channels at 24x24 = 3456 features
    self.l1 = nn.Linear(6 * 24 * 24, 10)
