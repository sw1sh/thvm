...
fn int cpu_dispatch_tile(KernelEntry *ke, u32 *in_buf_ids, u32 out_buf_id) {
  ...
  // Slice 8 session 5: the mma_tile_id != 0 bail retired with
  // TilePlanInfo.mma_tile_id; no matmul-shape gate fires here today.
...
// The post-F6 dispatch ladder (BLAS shape classifier / tile interp /
// UOp-walk / JIT / scalar-uops) all read inputs through
