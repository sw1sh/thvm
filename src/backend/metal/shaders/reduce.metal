...
kernel void thvm_reduce_f32(device         float *out      [[buffer(0)]],
                            ...
                            uint                  tg_size  [[threads_per_grid]])
{
    ...
    uint inner_idx = tid - outer_idx * inner;
...
kernel void thvm_reduce_i32(device         int   *out      [[buffer(0)]],
                            constant       uint  &arg      [[buffer(1)]],
                            ...
                            uint                  tg_size  [[threads_per_grid]])
{
    ...
    uint outer_idx = tid / inner;
    uint inner_idx = tid - outer_idx * inner;
    int acc = (kind == 1u) ? INT_MIN : 0;
    for (uint k = 0u; k < axis_size; k++) {
        int v = a[outer_idx * (axis_size * inner) + k * inner + inner_idx];
        acc = (kind == 1u) ? max(acc, v) : (acc + v);
    }
    out[tid] = acc;
}
