...
static int tile_analyze_conv2d_flat_impl(KernelEntry const *ke,
  ...
  // input_views-decouple session 1: try the DAG-side full-shape extractor
  // first.  When the lift took the single-input path AND the address tree
  // wasn't degenerated by the simplifier, the extractor recovers every
  // shape field from the W and X INDEX_E.addr trees.  This decouples the
  // conv2d analyser from `ke->input_views[]` for the common case (matches
  // session 2's matmul/dot/gemv pattern).  Multi-input conv (IWHERE chain)
  // and degenerate kh==kw==1 still fall through to the input_views reader
  // below.
